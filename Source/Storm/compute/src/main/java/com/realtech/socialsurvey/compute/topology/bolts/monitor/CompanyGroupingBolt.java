        ...
        if(!TupleUtils.isTick( input )) {
            ...
            if ( socialResponseType != null && socialResponseType.getType() != null ) {
                if ( socialResponseType.getType().equals( SocialFeedType.FACEBOOK ) ) {
                    post = ConversionUtils.deserialize( postString, new TypeToken<SocialResponseObject<FacebookFeedData>>()
                    {
                    }.getType() );
                } else if ( socialResponseType.getType().equals( SocialFeedType.TWITTER ) ) {
                    post = ConversionUtils.deserialize( postString, new TypeToken<SocialResponseObject<TwitterFeedData>>()
                    {
                    }.getType() );
                } else if ( socialResponseType.getType().equals( SocialFeedType.LINKEDIN ) ) {
                    post = ConversionUtils.deserialize( postString, new TypeToken<SocialResponseObject<LinkedinFeedData>>()
                    {
                    }.getType() );
                } else if ( socialResponseType.getType().equals( SocialFeedType.INSTAGRAM ) ) {
                    post = ConversionUtils.deserialize( postString, new TypeToken<SocialResponseObject<InstagramMediaData>>()
                    {
                    }.getType() );
                }
