...
public class ProfileManagementServiceImpl implements ProfileManagementService, InitializingBean {
	...
	@Override
	@Transactional
	public OrganizationUnitSettings getIndividualSettingsByProfileName(String agentProfileName) throws ProfileNotFoundException,
	...
	public SocialMediaTokens aggregateSocialProfiles(OrganizationUnitSettings unitSettings, String entity) throws InvalidInputException,
		...
		if (companySettings.getSocialMediaTokens() != null) {
			...
			if ((entityTokens.getRealtorToken().getRealtorProfileLink() == null || entityTokens.getRealtorToken().getRealtorProfileLink().equals(""))
	...
	public void addOrUpdateAgentPositions(List<CompanyPositions> companyPositions, AgentSettings agentSettings) {
		...
		organizationUnitSettingsDao.updateParticularKeyAgentSettings(MongoOrganizationUnitSettingDaoImpl.KEY_POSTIONS, companyPositions,
				agentSettings);
	}
	...
	public Map<SettingsForApplication, OrganizationUnit> getPrimaryHierarchyByEntity(String entityType, long entityId) throws InvalidInputException,
		...
		List<SettingsDetails> settingsDetailsList = settingsManager.getScoreForCompleteHeirarchy(companyId, branchId, regionId);
		...
		if (entityType.equalsIgnoreCase(CommonConstants.AGENT_ID)) {
			if (!settingsLocker.isSettingsValueLocked(OrganizationUnit.COMPANY, currentLockAggregateValue, SettingsForApplication.LOGO)) {
				if (!settingsLocker.isSettingsValueLocked(OrganizationUnit.REGION, currentLockAggregateValue, SettingsForApplication.LOGO)) {
					if (!settingsLocker.isSettingsValueLocked(OrganizationUnit.BRANCH, currentLockAggregateValue, SettingsForApplication.LOGO)) {
						logoLocked = false;
					}
				}
			}
			if (!settingsLocker.isSettingsValueLocked(OrganizationUnit.COMPANY, currentLockAggregateValue, SettingsForApplication.PHONE)) {
				if (!settingsLocker.isSettingsValueLocked(OrganizationUnit.REGION, currentLockAggregateValue, SettingsForApplication.PHONE)) {
					if (!settingsLocker.isSettingsValueLocked(OrganizationUnit.BRANCH, currentLockAggregateValue, SettingsForApplication.PHONE)) {
						phoneNumberLocked = false;
					}
				}
			}
			if (!settingsLocker.isSettingsValueLocked(OrganizationUnit.COMPANY, currentLockAggregateValue, SettingsForApplication.WEB_ADDRESS_WORK)) {
				if (!settingsLocker
						.isSettingsValueLocked(OrganizationUnit.REGION, currentLockAggregateValue, SettingsForApplication.WEB_ADDRESS_WORK)) {
		...
		if (entityType.equalsIgnoreCase(CommonConstants.AGENT_ID)) {
			if (unitSettings != null) {
				...
				if (unitSettings.getContact_details() != null) {
					if ((unitSettings.getContact_details().getAddress1() != null && !unitSettings.getContact_details().getAddress1().isEmpty())
							|| (unitSettings.getContact_details().getAddress2() != null && !unitSettings.getContact_details().getAddress2().isEmpty())) {
	...
	public OrganizationUnitSettings getBranchSettingsByProfileName(String companyProfileName, String branchProfileName)
			throws ProfileNotFoundException, InvalidInputException {
		LOG.info("Method getBranchSettingsByProfileName called for companyProfileName:" + companyProfileName + " and branchProfileName:"
				+ branchProfileName);
	...
	@Override
	@Transactional
	public OrganizationUnitSettings getRegionProfileByBranch(OrganizationUnitSettings branchSettings) throws ProfileNotFoundException {
	...
	public OrganizationUnitSettings fillUnitSettings(OrganizationUnitSettings unitSettings, String currentProfileName,
			OrganizationUnitSettings companyUnitSettings, OrganizationUnitSettings regionUnitSettings, OrganizationUnitSettings branchUnitSettings,
		...
		if (currentProfileName.equalsIgnoreCase(MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION)) {
			...
			regionUnitSettings = setAggregateProfileData(regionUnitSettings, companyUnitSettings, regionUnitSettings, null, null, map);
			...
			branchUnitSettings = setAggregateProfileData(branchUnitSettings, companyUnitSettings, regionUnitSettings, branchUnitSettings, null, map);
			...
			agentUnitSettings = setAggregateProfileData(agentUnitSettings, companyUnitSettings, regionUnitSettings, branchUnitSettings,
					agentUnitSettings, map);
	...
	private OrganizationUnitSettings setAggregateBasicData(OrganizationUnitSettings userProfile, OrganizationUnitSettings parentProfile) {
	...
	private OrganizationUnitSettings setAggregateProfileData(OrganizationUnitSettings userProfile, OrganizationUnitSettings companyUnitSettings,
			OrganizationUnitSettings regionUnitSettings, OrganizationUnitSettings branchUnitSettings, OrganizationUnitSettings agentUnitSettings,
	...
	public void updateEmailsWithLogo(OrganizationUnitSettings unitSettings, String logoUrl, String collectionName) {
		...
		if (mailContentSettings != null) {
			...
			if (takeSurveyMailContent != null) {
				...
				try {
					mailBody = organizationManagementService.readMailContentFromFile(CommonConstants.SURVEY_REQUEST_MAIL_FILENAME);
			...
			if (takeSureyMailCustomerContent != null) {
				...
				try {
					mailBody = organizationManagementService.readMailContentFromFile(CommonConstants.SURVEY_CUSTOMER_REQUEST_MAIL_FILENAME);
			...
			if (surveyReminderMailContent != null) {
				...
				try {
					mailBody = organizationManagementService.readMailContentFromFile(CommonConstants.SURVEY_REMINDER_MAIL_FILENAME);
	...
	private Map<String, Object> convertJsonStringToMap(String jsonString) throws JsonParseException, JsonMappingException, IOException {
