...
public class DotloopReviewProcessor extends QuartzJobBean
{
	...
	protected void executeInternal( JobExecutionContext jobExecutionContext )
	{
		try {
			...
			batchTrackerService
				.getLastRunEndTimeAndUpdateLastStartTimeByBatchType( CommonConstants.BATCH_TYPE_DOT_LOOP_REVIEW_PROCESSOR,
					CommonConstants.BATCH_NAME_DOT_LOOP_REVIEW_PROCESSOR );
			...
			if ( organizationUnitSettingsList != null && !organizationUnitSettingsList.isEmpty() ) {
				...
				for ( OrganizationUnitSettings organizationUnitSettings : organizationUnitSettingsList ) {
					...
					if ( dotLoopCrmInfo.getApi() != null && !dotLoopCrmInfo.getApi().isEmpty() ) {
						...
						crmBatchTrackerService
							.getLastRunEndTimeAndUpdateLastStartTimeByEntityTypeAndSourceType( entityType, entityId,
						...
						try {
							...
							crmBatchTracker = crmBatchTrackerService
								.getCrmBatchTracker( entityType, entityId, CommonConstants.CRM_SOURCE_DOTLOOP );
							...
							try {
								...
								String errorMsg =
									"Error while processing dotloop feed for collection " + collectionName + ", profile name "
										+ organizationUnitSettings.getProfileName() + " with iden " + organizationUnitSettings
										.getIden() + "at time " + new Date( System.currentTimeMillis() ) + " <br>";
								errorMsg += "<br>" + e.getMessage() + "<br><br>";
								errorMsg +=
									"<br>StackTrace : <br>" + ExceptionUtils.getStackTrace( e ).replaceAll( "\n", "<br>" )
										+ "<br>";
	...
	private List<DotLoopProfileEntity> getDotLoopProfiles( String authorizationHeader, String apiKey )
	{
		...
		if ( dotloopIntegrationApi != null ) {
			...
			if ( responseString != null ) {
				profiles = new Gson().fromJson( responseString, new TypeToken<List<DotLoopProfileEntity>>()
				{
				}.getType() );
			}
		}
			...
			try {
				...
				if ( !loop.getLoopStatus().equalsIgnoreCase( SOLD_STATUS ) ) {
					...
					throw new FatalException(
						"Found a loop status which is not sold. Details: Loop view id: " + loop.getLoopViewId()
							+ " for profile id: " + profileId + ". Collection: " + collectionName + " ID: "
							+ organizationUnitId );
				...
				try {
					loopFromSystem = organizationManagementService
						.getLoopByProfileAndLoopId( profileId, loop.getLoopId(), collectionName, organizationUnitId );
		...
		long organizationUnitId )
	{
		...
		try {
			response = dotloopIntegrationApi
				.fetchLoopViewParticipants( authorizationHeader, loop.getProfileId(), loop.getLoopViewId() );
			if ( response != null ) {
				responseString = new String( ( (TypedByteArray) response.getBody() ).getBytes() );
				participants = new Gson().fromJson( responseString, new TypeToken<List<DotLoopParticipant>>()
				{
				}.getType() );
				//JIRA SS-493 begin
				...
				if ( buyersMap != null && buyersMap.size() > 0 && buyingAgentEmailIdList != null && !buyingAgentEmailIdList
					.isEmpty() ) {
				...
				LOG.info( "No response fetched for loop details " + loop.getLoopViewId() + " for profile id: " + loop
					.getProfileId() );
			}
									...
									if ( loopResponse != null ) {
										...
										if ( loopResponseString == null || loopResponseString.equals( "[]" ) ) {
											...
											loopEntities = new Gson()
												.fromJson( loopResponseString, new TypeToken<List<LoopProfileMapping>>()
												{
												}.getType() );
