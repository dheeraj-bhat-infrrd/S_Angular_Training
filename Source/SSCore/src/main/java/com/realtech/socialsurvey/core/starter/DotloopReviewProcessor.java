...
public class DotloopReviewProcessor extends QuartzJobBean {
	...
	public void startDotloopFeedProcessing(String collectionName) {
		...
		try {
			List<OrganizationUnitSettings> organizationUnitSettingsList = organizationManagementService.getOrganizationUnitSettingsForCRMSource(
					CommonConstants.CRM_SOURCE_DOTLOOP, collectionName);
				...
				for (OrganizationUnitSettings organizationUnitSettings : organizationUnitSettingsList) {
					...
					if (dotLoopCrmInfo.getApi() != null && !dotLoopCrmInfo.getApi().isEmpty()) {
						...
						try {
							...
							LOG.error("Exception caught for collection " + collectionName + "having iden as " + organizationUnitSettings.getIden(), e);
	...
	private boolean isProfilePresentAsInactive(String collectionName, OrganizationUnitSettings unitSettings, DotLoopProfileEntity dotLoopProfile)
	...
	private void insertCompanyDotloopProfile(String collectionName, DotLoopProfileEntity profileEntity, OrganizationUnitSettings unitSettings)
			throws InvalidInputException {
		...
		collectionDotloopProfileMapping = organizationManagementService.saveCollectionDotLoopProfileMapping(collectionDotloopProfileMapping);
	}

	private void processLoopEntites(String collectionName, List<LoopProfileMapping> loops, String profileId, boolean byPassRecords,
			String authorizationHeader, long organizationUnitId) {
		...
		for (LoopProfileMapping loop : loops) {
			...
			if (!loop.getLoopStatus().equalsIgnoreCase(SOLD_STATUS)) {
				...
				throw new FatalException("Found a loop status which is not sold. Details: Loop view id: "+loop.getLoopViewId()+" for profile id: "+profileId+". Collection: "+collectionName+" ID: "+organizationUnitId);
			}
			// check if the record is present in the database then skip the loop. if not, then
			// process it
			LoopProfileMapping loopFromSystem = null;
			try {
				loopFromSystem = organizationManagementService.getLoopByProfileAndLoopId(profileId, loop.getLoopId(), collectionName,
						organizationUnitId);
			}
	...
	private LoopProfileMapping setHierarchyInformationInLoop(LoopProfileMapping loop, String collectionName, long organizationUnitId) {
	...
	private void processLoop(String collectionName, LoopProfileMapping loop, String authorizationHeader, long organizationUnitId) {
	...
	private SurveyPreInitiation setCollectionDetails(SurveyPreInitiation surveyPreInitiation, String collectionName, long organizationUnitId) {
	...
	private void updateDotLoopCrmInfo(String collectionName, OrganizationUnitSettings unitSettings, DotLoopCrmInfo dotLoopCrmInfo) {
	 ...
	 * @param unitSettings
	 */
	public void fetchReviewfromDotloop(DotLoopCrmInfo dotloopCrmInfo, String collectionName, OrganizationUnitSettings unitSettings) {
				...
				try {
					if (profile.isActive() && !isProfilePresentAsInactive(collectionName, unitSettings, profile)) {
						...
						try {
							...
							if (dotloopCrmInfo.isRecordsBeenFetched()
									&& organizationManagementService.getLoopsCountByProfile(profileId, collectionName, unitSettings.getIden()) > 0) {
							...
							do {
								...
								if (loopResponse != null) {
									...
									if (loopResponseString == null || loopResponseString.equals("[]")) {
										...
										processLoopEntites(collectionName, loopEntities, profileId, byPassRecords, authorizationHeader,
