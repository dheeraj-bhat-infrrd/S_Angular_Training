...
public class UserManagementServiceImpl implements UserManagementService, InitializingBean
{
    ...
    private void makeAProfileAsPrimaryOfAUser(long userId) throws InvalidInputException, ProfileNotFoundException
    {
		...
		String errorMessage = "No primary user profile found for user " + user.getFirstName() + user .getLastName() + ", with user id : " + user.getUserId();
		try {
		...
		LOG.debug("marked a profile with profile id " + profileToMakePrimary.getUserProfileId() + "  as primary for user with user id " + user.getUserId());
