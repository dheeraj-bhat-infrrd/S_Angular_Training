...
public class SocialManagementServiceImpl implements SocialManagementService, InitializingBean {
				...
				if (settings.getSocialMediaTokens().getFacebookToken() != null
					...
					if (settings.getSocialMediaTokens().getFacebookToken().getFacebookAccessTokenToPost() != null)
						facebook.setOAuthAccessToken(new AccessToken(
								settings.getSocialMediaTokens().getFacebookToken().getFacebookAccessTokenToPost(),
								null));
					else
						facebook.setOAuthAccessToken(new AccessToken(
								settings.getSocialMediaTokens().getFacebookToken().getFacebookAccessToken(), null));
					try {
						...
						if (companyId == Long.parseLong(customisedSocialNetworkCompanyId)) {
							try {
								postUpdate.setPicture(
										new URL("https://don7n2as2v6aa.cloudfront.net/remax-facebook-image.png"));
	...
	public boolean tweet(OrganizationUnitSettings agentSettings, String message, long companyId)
		...
		if (agentSettings != null) {
			if (agentSettings.getSocialMediaTokens() != null) {
				if (agentSettings.getSocialMediaTokens().getTwitterToken() != null && agentSettings
						.getSocialMediaTokens().getTwitterToken().getTwitterAccessTokenSecret() != null) {
					Twitter twitter = getTwitterInstance();
	...
	public boolean updateLinkedin(OrganizationUnitSettings settings, String collectionName, String message,
			String linkedinProfileUrl, String linkedinMessageFeedback, OrganizationUnitSettings companySettings,
			boolean isZillow, AgentSettings agentSettings, SocialMediaPostResponse linkedinPostResponse,
			String surveyId) throws NonFatalException {
		if (settings == null) {
			throw new InvalidInputException("AgentSettings can not be null");
		}
				...
				if (settings.getSocialMediaTokens().getLinkedInToken() != null
					...
					try {
						...
						String description = "Reviews for " + agentContactDetailsSettings.getName() + ". "
								+ agentContactDetailsSettings.getFirstName() + " is a  " + industry
								+ " professional in " + (location != null && !location.isEmpty() ? location : "") + ". "
						...
						if (agentSettings.getProfileImageUrl() != null
								&& !agentSettings.getProfileImageUrl().isEmpty()) {
						...
						String linkedPostJSON = "{\"comment\": \"" + message + "\",\"content\": {" + "\"title\": \""
								+ title + "\"," + "\"description\": \"" + description + "\"," + "\"submitted-url\": \""
								+ profileUrl + "\",  " + "\"submitted-image-url\": \"" + imageUrl + "\"},"
								+ "\"visibility\": {\"code\": \"anyone\" }}";
						...
						try {
							...
							JSONObject entityUpdateResponseObj = new JSONObject(
							...
							if (responseString.contains("201 Created")) {
								...
								linkedinPostResponse
										.setResponseMessage((String) entityUpdateResponseObj.get("message"));
							}
							linkedinPostResponse.setAccessToken(
									settings.getSocialMediaTokens().getLinkedInToken().getLinkedInAccessToken());
						} catch (RuntimeException e) {
							LOG.error(
									"Runtime exception caught while trying to add an update on linkedin. Nested exception is ",
									e);
						}
					} catch (IOException e) {
						throw new NonFatalException(
								"IOException caught while posting on Linkedin. Nested exception is ", e);
					}
				}
			}
		}
	...
	public Map<String, List<OrganizationUnitSettings>> getSettingsForBranchesAndRegionsInHierarchy(long agentId)
		...
		companySettings.add(organizationUnitSettingsDao.fetchOrganizationUnitSettingsById(
				user.getCompany().getCompanyId(), CommonConstants.COMPANY_SETTINGS_COLLECTION));
	...
	@Override
	@Transactional
	public Map<String, List<OrganizationUnitSettings>> getSettingsForBranchesRegionsAndCompanyInAgentsHierarchy(
			long agentId) throws InvalidInputException {
		LOG.info(
				"Method to get settings of branches and regions current agent belongs to, getSettingsForBranchesAndRegionsInHierarchy() started.");
		User user = userDao.findById(User.class, agentId);
		Map<String, List<OrganizationUnitSettings>> map = new HashMap<String, List<OrganizationUnitSettings>>();
	...
	void removeFromSocialPosts(String collectionName, long entityId, String source) throws InvalidInputException {
		LOG.info("Method to remove social posts started for collectionName : " + collectionName + " entityId : "
		...
		LOG.info("Method to remove social posts finished for collectionName : " + collectionName + " entityId : "
				+ entityId + " source : " + source);
	}
			...
			if (profile != null) {
				...
				if (branch.getIsDefaultBySystem() == CommonConstants.IS_DEFAULT_BY_SYSTEM_NO) {
					primaryProfileSetting = organizationManagementService
							.getBranchSettingsDefault(branch.getBranchId());
	 ...
	 */
	@Override
	public void postToFacebookForHierarchy(String facebookMessage, double rating, String serverBaseUrl,
			int accountMasterId, SocialMediaPostDetails socialMediaPostDetails,
			SocialMediaPostResponseDetails socialMediaPostResponseDetails, boolean isZillow, boolean isAgentsHidden)
			...
			if (socialMediaPostDetails.getCompanyMediaPostDetails() != null) {
				...
				if (companySetting != null) {
					...
					if (company != null && company.getStatus() != CommonConstants.STATUS_INACTIVE) {
						postToFacebookForAHierarchy(companyId, agentId, profileurl, facebookMessage,
								updatedFacebookMessage, rating, serverBaseUrl, companySetting,
					...
					if (setting != null) {
						...
						if (region != null && region.getStatus() != CommonConstants.STATUS_INACTIVE) {
							...
							postToFacebookForAHierarchy(companyId, agentId, profileurl, facebookMessage,
									...
									regionMediaPostDetails, regionMediaPostResponseDetails, isZillow, isAgentsHidden);
						}
					}
					...
					if (branch != null && branch.getStatus() != CommonConstants.STATUS_INACTIVE) {
						if (setting != null) {
							postToFacebookForAHierarchy(companyId, agentId, profileurl, facebookMessage,
									...
									branchMediaPostDetails, branchMediaPostResponseDetails, isZillow, isAgentsHidden);
						}
					}
				}
	...
	void postToFacebookForAHierarchy(long companyId, long agentId, String profileUrl, String facebookMessage,
			...
			String collectionType, MediaPostDetails mediaPostDetails,
			EntityMediaPostResponseDetails mediaPostResponseDetails, boolean isZillow, boolean isAgentsHidden)
			throws InvalidInputException {
		try {
			...
			if (surveyHandler.canPostOnSocialMedia(setting, rating)) {
				...
				if (!updateStatusIntoFacebookPage(setting, updatedFacebookMessage, serverBaseUrl, companyId,
					...
					facebookPostResponse.setAccessToken(
							setting.getSocialMediaTokens().getFacebookToken().getFacebookAccessTokenToPost());
	...
	public void postToLinkedInForHierarchy(String linkedinMessage, double rating, String linkedinProfileUrl,
			...
			boolean isZillow, boolean isAgentsHidden, String surveyId)
			throws InvalidInputException, NoRecordsFetchedException {
					...
					if (setting != null) {
						...
						if (region != null && region.getStatus() != CommonConstants.STATUS_INACTIVE) {
							postToLinkedInForAHierarchy(setting,
									...
									updatedLinkedInMessage, linkedinMessage, linkedinProfileUrl,
									linkedinMessageFeedback, companySettings, agentSettings, regionMediaPostDetails,
									regionMediaPostResponseDetails, surveyId);
						}
					}
				}
			}
					...
					if (setting != null) {
						...
						if (branch != null && branch.getStatus() != CommonConstants.STATUS_INACTIVE) {
							postToLinkedInForAHierarchy(setting,
									...
									updatedLinkedInMessage, linkedinMessage, linkedinProfileUrl,
									linkedinMessageFeedback, companySettings, agentSettings, branchMediaPostDetails,
									branchMediaPostResponseDetails, surveyId);
						}
					}
				}
			}
	...
	void postToLinkedInForAHierarchy(OrganizationUnitSettings setting, String collectionName, Double rating,
			boolean isZillow, String updatedLinkedInMessage, String linkedinMessage, String linkedinProfileUrl,
			String linkedinMessageFeedback, OrganizationUnitSettings companySettings, AgentSettings agentSettings,
			MediaPostDetails mediaPostDetails, EntityMediaPostResponseDetails mediaPostResponseDetails,
			String surveyId) {
		try {
			if (surveyHandler.canPostOnSocialMedia(setting, rating)) {
				if (!isZillow) {
			...
			SocialMediaPostResponse linkedinPostResponse = new SocialMediaPostResponse();
			linkedinPostResponse
					.setAccessToken(setting.getSocialMediaTokens().getLinkedInToken().getLinkedInAccessToken());
			linkedinPostResponse.setPostDate(new Date(System.currentTimeMillis()));
	 ...
	 */
	@Override
	public void postToTwitterForHierarchy(String twitterMessage, double rating, String serverBaseUrl,
			int accountMasterId, SocialMediaPostDetails socialMediaPostDetails,
			SocialMediaPostResponseDetails socialMediaPostResponseDetails, boolean isAgentsHidden)
			throws InvalidInputException, NoRecordsFetchedException {
			...
			for (RegionMediaPostDetails regionMediaPostDetails : socialMediaPostDetails
				...
				if (regionMediaPostDetails != null) {
					...
					if (setting != null) {
						...
						Region region = userManagementService.getRegionById(setting.getIden());
						if (region != null && region.getStatus() != CommonConstants.STATUS_INACTIVE) {
							postToTwitterForAHierarchy(setting, rating, companyId, twitterMessage,
	...
	public void postToTwitterForAHierarchy(OrganizationUnitSettings setting, Double rating, long companyId,
			String twitterMessage, MediaPostDetails mediaPostDetails,
			EntityMediaPostResponseDetails mediaPostResponseDetails) throws InvalidInputException {
		try {
			if (surveyHandler.canPostOnSocialMedia(setting, rating)) {
	 ...
	 */
	@Override
	public boolean postToSocialMedia(String agentName, String agentProfileLink, String custFirstName,
			String custLastName, long agentId, double rating, String surveyId, String feedback, boolean isAbusive,
			String serverBaseUrl, boolean onlyPostToSocialSurvey) throws NonFatalException {
		...
		try {
			...
			try {
				...
				LOG.error(
						"NullPointerException caught in postToSocialMedia() while fetching account master id for agent "
								+ agent.getFirstName());
			...
			SocialMediaPostDetails socialMediaPostDetails = surveyHandler.getSocialMediaPostDetailsBySurvey(
					surveyDetails, companySettings.get(0), regionSettings, branchSettings);
			...
			if (agentMediaPostResponseDetails == null) {
				...
				agentMediaPostResponseDetails
						.setAgentId(socialMediaPostDetails.getAgentMediaPostDetails().getAgentId());
			}
			CompanyMediaPostResponseDetails companyMediaPostResponseDetails = socialMediaPostResponseDetails
					.getCompanyMediaPostResponseDetails();
			...
			if (!isAbusive && !onlyPostToSocialSurvey) {
				...
				String facebookMessage = buildFacebookAutoPostMessage(customerDisplayName, agentName, rating, feedback,
						surveyHandler.getApplicationBaseUrl() + CommonConstants.AGENT_PROFILE_FIXED_URL
								+ agentProfileLink + "/",
						false);

				postToFacebookForHierarchy(facebookMessage, rating, serverBaseUrl, accountMasterId,
						socialMediaPostDetails, socialMediaPostResponseDetails, false, isCompanyAgentHidden);
				...
				String linkedinProfileUrl = surveyHandler.getApplicationBaseUrl()
				...
				postToLinkedInForHierarchy(linkedinMessage, rating, linkedinProfileUrl, linkedinMessageFeedback,
						accountMasterId, socialMediaPostDetails, socialMediaPostResponseDetails, companySettings.get(0),
						false, isCompanyAgentHidden, surveyId);
				...
				if (profileUrlWithMessage == null || profileUrlWithMessage.isEmpty()) {
					profileUrlWithMessage = surveyHandler.getApplicationBaseUrl()
							+ CommonConstants.AGENT_PROFILE_FIXED_URL + agentProfileLink;
				}
				...
				postToTwitterForHierarchy(twitterMessage, rating, serverBaseUrl, accountMasterId,
						socialMediaPostDetails, socialMediaPostResponseDetails, isCompanyAgentHidden);
	...
	public ExternalSurveyTracker checkExternalSurveyTrackerExist(String entityColumnName, long entityId, String source,
		...
		return externalSurveyTrackerDao.checkExternalSurveyTrackerDetailsExist(entityColumnName, entityId, source,
				reviewUrl, reviewDate);
	}
	...
	public void saveExternalSurveyTracker(String entityColumnName, long entityId, String source, String sourceLink,
			...
			String postedOn) {
		externalSurveyTrackerDao.saveExternalSurveyTracker(entityColumnName, entityId, source, sourceLink, reviewUrl,
				rating, autoPostStatus, complaintResolutionStatus, reviewDate, postedOn);
	}
	...
	public SurveyPreInitiationList getUnmatchedPreInitiatedSurveys(long companyId, int startIndex, int batchSize,
			long count) throws InvalidInputException {
		...
		List<SurveyPreInitiation> surveyPreInitiations = surveyPreInitiationDao
				.getUnmatchedPreInitiatedSurveys(companyId, startIndex, batchSize);
		surveyPreInitiationListVO.setSurveyPreInitiationList(surveyPreInitiations);
		// function shd be called only once
		if (count == -1) {
			surveyPreInitiationListVO
					.setTotalRecord(surveyPreInitiationDao.getUnmatchedPreInitiatedSurveyCount(companyId));
		} else {
	...
	public SurveyPreInitiationList getProcessedPreInitiatedSurveys(long companyId, int startIndex, int batchSize,
		...
		List<SurveyPreInitiation> surveyPreInitiations = surveyPreInitiationDao
				.getProcessedPreInitiatedSurveys(companyId, startIndex, batchSize);
		surveyPreInitiationListVO.setSurveyPreInitiationList(surveyPreInitiations);
		if (count == -1) {
			surveyPreInitiationListVO
					.setTotalRecord(surveyPreInitiationDao.getProcessedPreInitiatedSurveyCount(companyId));
		} else {
			surveyPreInitiationListVO.setTotalRecord(count);
		}
		return surveyPreInitiationListVO;
	...
	public String buildFacebookAutoPostMessage(String customerDisplayName, String agentName, double rating,
		...
		String facebookMessage = rating + " Star Review on " + (isZillow ? "Zillow" : "SocialSurvey") + " \u2014 "
	...
	public String buildLinkedInAutoPostMessage(String customerDisplayName, String agentName, double rating,
		...
		String linkedinMessage = rating + " Star Review on " + (isZillow ? "Zillow" : "SocialSurvey") + " &dash; "
				+ linkedInComment + " by " + customerDisplayName + " for " + agentName + "&lmnlf;"
				+ (isZillow ? linkUrl : "");
	...
	@Override
	public Map<Long, List<SocialUpdateAction>> getSocialConnectionsHistoryForEntities(String entityType,
			List<Long> entityIds) throws InvalidInputException, ProfileNotFoundException {
		// Check if any of the parameters are null or empty
		if (entityType == null || entityType.isEmpty()) {
			throw new InvalidInputException("Invalid entity type. EntityType : " + entityType);
		}
	...
	public SurveyPreInitiationList getCorruptPreInitiatedSurveys(long companyId, int startIndex, int batchSize,
		...
		if (count == -1) {
			surveyPreInitiationListVO
					.setTotalRecord(surveyPreInitiationDao.getCorruptPreInitiatedSurveyCount(companyId));
		} else {
			surveyPreInitiationListVO.setTotalRecord(count);
		}
		return surveyPreInitiationListVO;
		...
		try {
			...
			for (Company company : organizationManagementService.getAllCompanies()) {
				...
				for (SurveyDetails survey : incompleteSocialPostCustomers) {
					...
					if (socialSitesWithSettings.get(CommonConstants.ZILLOW_LABEL) != null) {
						links.append("<a " + STYLE_ATTR + " href="
								+ socialSitesWithSettings.get(CommonConstants.ZILLOW_LABEL) + ">"
								+ CommonConstants.ZILLOW_LABEL + "</a>");
					}
					if (socialSitesWithSettings.get(CommonConstants.YELP_LABEL) != null) {
						links.append(
								"<a " + STYLE_ATTR + " href=" + socialSitesWithSettings.get(CommonConstants.YELP_LABEL)
					...
					if (!links.toString().isEmpty()) {
						try {
							surveyHandler.sendSocialPostReminderMail(survey.getCustomerEmail(),
									survey.getCustomerFirstName(), survey.getCustomerLastName(), user,
									links.toString());
							surveyHandler.updateReminderCountForSocialPosts(survey.getAgentId(),
									survey.getCustomerEmail());
							...
							LOG.error(
									"Exception caught in IncompleteSurveyReminderSender.main while trying to send reminder mail to "
											+ survey.getCustomerFirstName()
											+ " for completion of survey. Nested exception is ",
									e);
							continue;
						}
					}
			...
			try {
				...
				batchTrackerService.sendMailToAdminRegardingBatchError(
						CommonConstants.BATCH_NAME_INCOMPLETE_SOCIAL_POST_REMINDER_SENDER, System.currentTimeMillis(),
						e);
	...
	private Map<String, String> getSocialSitesWithSettingsConfigured(SurveyDetails survey)
			throws InvalidInputException {
		LOG.debug(
	...
	private String generateSocialSiteUrl(SurveyDetails survey, String socialSite,
		...
		String url = "";
		String customerDisplayName = new EmailFormatHelper()
				.getCustomerDisplayNameForEmail(survey.getCustomerFirstName(), survey.getCustomerLastName());
		...
		case CommonConstants.LINKEDIN_LABEL:
			url += "https://www.linkedin.com/shareArticle?mini=true&url="
					+ organizationUnitSettings.getCompleteProfileUrl() + "/" + survey.get_id() + "&title=&summary="
					+ reviewText + "&source=";
			break;
		...
		try {
			...
			for (Company company : organizationManagementService.getAllCompanies()) {
				try {
					...
					do {
						batchUserIdList = organizationManagementService.getAgentIdsUnderCompany(companyId, start,
								batchSize);
					...
					// Fetch all zillow data from temp table and trigger auto
					// post
						...
						for (ZillowTempPost zillowTempPost : zillowTempPostList) {
							try {
								if (zillowTempPost != null) {
									...
									// TODO : remove this . Temporary fix for
									// Zillow review URl
									...
									// change this to support another units in
									// hierarchy
									...
									if (checkReviewCanBePostedToSocialMedia(zillowTempPost, agentSetting,
											companySettings, surveyDetails)) {
										// post the zillow review to social
										// media
										List<String> postedOnList = null;
										boolean autoPostSuccess = false;
										String postedOn = "";
										try {
											postedOnList = postToSocialMedia(zillowTempPost, agentSetting,
													surveyDetails, agentSetting);
										} catch (Exception e) {
										...
										if (!complaintResStatus && autoPostSuccess) {
											...
											saveExternalSurveyTracker(zillowTempPost.getEntityColumnName(),
													...
													agentSetting.getSocialMediaTokens().getZillowToken()
															.getZillowProfileLink(),
													zillowTempPost.getZillowReviewUrl(),
													zillowTempPost.getZillowReviewRating(), postToSocialMedia,
													CommonConstants.NO, zillowTempPost.getZillowReviewDate(), postedOn);
										}
									}
									// add to zillow temp post id to processed
									// list
			...
			try {
				...
				// send report bug mail to admin
				batchTrackerService.sendMailToAdminRegardingBatchError(
						CommonConstants.BATCH_NAME_ZILLOW_REVIEW_PROCESSOR_AND_AUTO_POSTER, System.currentTimeMillis(),
						e);
			} catch (NoRecordsFetchedException | InvalidInputException e1) {
	...
	boolean checkReviewCanBePostedToSocialMedia(ZillowTempPost zillowTempPost, OrganizationUnitSettings unitSettings,
		...
		if (checkExternalSurveyTrackerExist(zillowTempPost.getEntityColumnName(), zillowTempPost.getEntityId(),
				...
				&& !utils.checkReviewForSwearWords(zillowTempPost.getZillowReviewDescription(),
						surveyHandler.getSwearList())
	...
	public List<String> postToSocialMedia(ZillowTempPost zillowTempPost,
			OrganizationUnitSettings organizationUnitSettings, SurveyDetails surveyDetails,
			OrganizationUnitSettings agentSettings) throws NonFatalException {
		...
		try {
			...
			if (zillowTempPost.getEntityColumnName().equalsIgnoreCase(CommonConstants.AGENT_ID_COLUMN)) {
				...
				int accountMasterId = 0;
				try {
					AccountsMaster masterAccount = agent.getCompany().getLicenseDetails()
							.get(CommonConstants.INITIAL_INDEX).getAccountsMaster();
					accountMasterId = masterAccount.getAccountsMasterId();
				} catch (NullPointerException e) {
					LOG.error(
							"NullPointerException caught in postToSocialMedia() while fetching account master id for agent "
				...
				socialMediaPostResponseDetails
						.setBranchMediaPostResponseDetailsList(branchMediaPostResponseDetailsList);
				socialMediaPostResponseDetails
						.setRegionMediaPostResponseDetailsList(regionMediaPostResponseDetailsList);
				...
				boolean isCompanyAgentHidden = companySetting.isHiddenSection();
				if (isCompanyAgentHidden) {
					agentName = commonUtils.getAgentNameForHiddenAgentCompany(agent.getFirstName(),
							agent.getLastName());
				}
				...
				postToLinkedInForHierarchy(linkedinMessage, zillowTempPost.getZillowReviewRating(), linkedinProfileUrl,
						linkedinMessageFeedback, accountMasterId, socialMediaPostDetails,
						socialMediaPostResponseDetails, companySettings.get(0), true, isCompanyAgentHidden, null);
				...
				if (agentSocialList != null && agentSocialList.size() > 0) {
				...
				} else if (socialMediaPostDetails != null
						&& socialMediaPostDetails.getRegionMediaPostDetailsList() != null) {
					for (RegionMediaPostDetails regionMediaPostDetailsList : socialMediaPostDetails
						...
						&& socialMediaPostDetails.getBranchMediaPostDetailsList() != null) {
					for (BranchMediaPostDetails branchMediaPostDetails : socialMediaPostDetails
							.getBranchMediaPostDetailsList()) {
	...
	private boolean triggerComplaintResolutionWorkflowForZillowReview(OrganizationUnitSettings companySettings,
			...
			int autoPostSuccess, String postedOn) {
		LOG.info(
			...
			if (complaintRegistrationSettings.isEnabled()
				...
				try {
					...
					emailServices.sendZillowReviewComplaintHandleMail(complaintRegistrationSettings.getMailId(),
							...
							String.valueOf(zillowTempPost.getZillowReviewRating()),
							zillowTempPost.getZillowReviewUrl());

					// add complaint resolution status in External Survey
					// Tracker
					saveExternalSurveyTracker(zillowTempPost.getEntityColumnName(), zillowTempPost.getEntityId(),
							...
							autoPostSuccess, CommonConstants.YES, zillowTempPost.getZillowReviewDate(), postedOn);
					return true;
	...
	public void updateFacebookToken(String collectionName, long iden, FacebookToken facebookToken) {
		...
		organizationUnitSettingsDao.updateParticularKeyOrganizationUnitSettingsByIden(
				MongoOrganizationUnitSettingDaoImpl.KEY_FACEBOOK_SOCIAL_MEDIA_TOKEN, facebookToken, iden,
				collectionName);
	...
	public void updateLinkedinToken(String collectionName, long iden, LinkedInToken linkedInToken) {
		...
		organizationUnitSettingsDao.updateParticularKeyOrganizationUnitSettingsByIden(
				MongoOrganizationUnitSettingDaoImpl.KEY_LINKEDIN_SOCIAL_MEDIA_TOKEN, linkedInToken, iden,
				collectionName);
	...
	@Override
	public String getClientCompanyProfileUrlForAgentToPostInSocialMedia(Long agentId,
			OrganizationUnitSettings unitSettings, String collectionType) throws InvalidInputException {
