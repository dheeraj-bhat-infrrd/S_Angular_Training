...
public class SettingsSetterImpl implements SettingsSetter
{
    ...
    @Override
    public Company setSettingsValueForCompany( Company company, SettingsForApplication settings, boolean hasBeenSet )
        throws NonFatalException
    {
        ...
        long modifiedSetSettingsValue = getModifiedSetSettingsValue( OrganizationUnit.COMPANY,
            Long.parseLong( company.getSettingsSetStatus() ), settings, hasBeenSet );
    ...
    @Override
    public Region setSettingsValueForRegion( Region region, SettingsForApplication settings, boolean hasBeenSet )
        throws NonFatalException
    {
        // Get the set settings value for the corresponding unit and set the value according to the
        // input sent
        ...
        long modifiedSetSettingsValue = getModifiedSetSettingsValue( OrganizationUnit.REGION,
            Long.parseLong( region.getSettingsSetStatus() ), settings, hasBeenSet );
    ...
    @Override
    public Branch setSettingsValueForBranch( Branch branch, SettingsForApplication settings, boolean hasBeenSet )
        throws NonFatalException
    {
        // Get the set settings value for the corresponding unit and set the value according to the
        // input sent
        ...
        long modifiedSetSettingsValue = getModifiedSetSettingsValue( OrganizationUnit.BRANCH,
            Long.parseLong( branch.getSettingsSetStatus() ), settings, hasBeenSet );
        branch.setSettingsSetStatus( String.valueOf( modifiedSetSettingsValue ) );
        return branch;
    }


    private long getModifiedSetSettingsValue( OrganizationUnit organizationUnit, long currentSetValue,
        SettingsForApplication settings, boolean hasBeenSet )
    {
    ...
    public boolean checkSettingsSetStatus( int settingNumber, OrganizationUnit organizationUnit )
    {
        ...
        if ( organizationUnit == OrganizationUnit.COMPANY ) {
            if ( settingNumber == CommonConstants.SET_BY_COMPANY || settingNumber == CommonConstants.SET_BY_COMPANY_N_REGION
                || settingNumber == CommonConstants.SET_BY_COMPANY_N_BRANCH
                || settingNumber == CommonConstants.SET_BY_COMPANY_N_REGION_N_BRANCH ) {
                setStatus = true;
            }
        } else if ( organizationUnit == OrganizationUnit.REGION ) {
            if ( settingNumber == CommonConstants.SET_BY_REGION || settingNumber == CommonConstants.SET_BY_COMPANY_N_REGION
                || settingNumber == CommonConstants.SET_BY_REGION_N_BRANCH
                || settingNumber == CommonConstants.SET_BY_COMPANY_N_REGION_N_BRANCH ) {
                setStatus = true;
            }
        } else if ( organizationUnit == OrganizationUnit.BRANCH ) {
