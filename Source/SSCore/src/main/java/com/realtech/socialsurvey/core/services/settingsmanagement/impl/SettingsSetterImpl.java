...
public class SettingsSetterImpl implements SettingsSetter {
	...
	public boolean checkSettingsSetStatus(int settingNumber, OrganizationUnit organizationUnit) {
		...
		if (organizationUnit == OrganizationUnit.COMPANY) {
			if (settingNumber == CommonConstants.SET_BY_COMPANY || settingNumber == CommonConstants.SET_BY_COMPANY_N_REGION || settingNumber == CommonConstants.SET_BY_COMPANY_N_BRANCH || settingNumber == CommonConstants.SET_BY_COMPANY_N_REGION_N_BRANCH) {
				setStatus = true;
			}
		}
		else if (organizationUnit == OrganizationUnit.REGION) {
			if (settingNumber == CommonConstants.SET_BY_REGION || settingNumber == CommonConstants.SET_BY_REGION_N_BRANCH) {
				setStatus = true;
			}
		}
		else if (organizationUnit == OrganizationUnit.BRANCH) {
			if (settingNumber == CommonConstants.SET_BY_BRANCH) {
				setStatus = true;
			}
		}
		return setStatus;
