...
public class SettingsLockerImpl implements SettingsLocker
{
    ...
    @Override
    public Company lockSettingsValueForCompany( Company company, SettingsForApplication settings, boolean hasBeenLocked )
        throws NonFatalException
    {
        ...
        LOG.info( "Setting lock values for company: " + company.getCompanyId() + " for settings " + settings
            + " with lock status: " + hasBeenLocked );
        long modifiedSettingsLockValue = getModifiedSetSettingsValue( OrganizationUnit.COMPANY,
            Long.parseLong( company.getSettingsLockStatus() ), settings, hasBeenLocked );
    ...
    @Override
    public Region lockSettingsValueForRegion( Region region, SettingsForApplication settings, boolean hasBeenLocked )
        throws NonFatalException
    {
        ...
        LOG.info( "Setting lock values for region: " + region.getRegionId() + " for settings " + settings
            + " with lock status: " + hasBeenLocked );
        long modifiedSettingsLockValue = getModifiedSetSettingsValue( OrganizationUnit.REGION,
            Long.parseLong( region.getSettingsLockStatus() ), settings, hasBeenLocked );
    ...
    @Override
    public Branch lockSettingsValueForBranch( Branch branch, SettingsForApplication settings, boolean hasBeenLocked )
        throws NonFatalException
    {
        ...
        LOG.info( "Setting lock values for branch: " + branch.getBranchId() + " for settings " + settings
            + " with lock status: " + hasBeenLocked );
        long modifiedSettingsLockValue = getModifiedSetSettingsValue( OrganizationUnit.BRANCH,
            Long.parseLong( branch.getSettingsLockStatus() ), settings, hasBeenLocked );
    ...
    @Override
    public boolean isSettingsValueLocked( OrganizationUnit organizationUnit, long currentLockValue,
    ...
    public boolean checkSettingsLockStatus( int lockNumber, OrganizationUnit organizationUnit )
    {
        ...
        if ( organizationUnit == OrganizationUnit.COMPANY ) {
            if ( lockNumber == CommonConstants.SET_BY_COMPANY || lockNumber == CommonConstants.SET_BY_COMPANY_N_REGION
                || lockNumber == CommonConstants.SET_BY_COMPANY_N_BRANCH
                || lockNumber == CommonConstants.SET_BY_COMPANY_N_REGION_N_BRANCH ) {
                lockStatus = true;
            }
            ...
            if ( lockNumber == CommonConstants.SET_BY_REGION || lockNumber == CommonConstants.SET_BY_COMPANY_N_REGION
                || lockNumber == CommonConstants.SET_BY_REGION_N_BRANCH
                || lockNumber == CommonConstants.SET_BY_COMPANY_N_REGION_N_BRANCH ) {
                lockStatus = true;
            }
    ...
    private long getModifiedSetSettingsValue( OrganizationUnit organizationUnit, long currentLockValue,
        SettingsForApplication settings, boolean hasBeenLocked ) throws InvalidSettingsStateException
    {
