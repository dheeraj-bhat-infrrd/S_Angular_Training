...
public class TopicConsumer implements Runnable {
		...
		if (stream != null) {
			...
			while (itrConsumer.hasNext()) {
				...
				try {
					...
					// 2 is for $$
					message = message.substring(HEADER_MARKER.length()
							+ header.length() + 2);
	...
	private String fetchHeader(String message) throws NonFatalException {
		...
		if (message.indexOf(HEADER_MARKER) == -1) {
			throw new InvalidMessageFormatException(
					"Invalid format of the message. No header found in the message.");
		}

		String header = message.substring(HEADER_MARKER.length(),
				message.indexOf(ELEMENTS_DELIMITER));
	...
	private void parseRegistrationMailMessage(String message)
			throws NonFatalException {
		LOG.debug("Registration mail message: " + message);
		if (message.indexOf(RECIPIENT_MARKER) == -1
				|| message.indexOf(URL_MARKER) == -1
				|| message.indexOf(FIRSTNAME_MARKER) == -1
				|| message.indexOf(LASTNAME_MARKER) == -1) {
			throw new InvalidMessageFormatException(
					"Invalid format for registration mail");
		}

		String recipient = message.substring(RECIPIENT_MARKER.length(),
				message.indexOf(ELEMENTS_DELIMITER));
		LOG.debug("Recipient: " + recipient);

		// holds the index till the message has been parsed.
		int messageParsedIndex = RECIPIENT_MARKER.length() + recipient.length()
				+ ELEMENTS_DELIMITER.length();
		String url = message.substring(
				messageParsedIndex + URL_MARKER.length(),
				message.indexOf(ELEMENTS_DELIMITER, messageParsedIndex));
		LOG.debug("Url: " + url);

		messageParsedIndex += URL_MARKER.length() + url.length()
				+ ELEMENTS_DELIMITER.length();
		String firstName = message.substring(
				messageParsedIndex + NAME_MARKER.length(),
				message.indexOf(ELEMENTS_DELIMITER, messageParsedIndex));
		LOG.debug("First Name: " + firstName);

		messageParsedIndex += FIRSTNAME_MARKER.length() + firstName.length()
				+ ELEMENTS_DELIMITER.length();
		String lastName = message.substring(messageParsedIndex
				+ LASTNAME_MARKER.length());
		LOG.debug("Last Name: " + lastName);

		LOG.debug("Sending registration invite mail");
		emailServices.sendRegistrationInviteMail(url, recipient, firstName,
				lastName);
	}

	private void parseMailMessage(String message, EmailHeader header)
			throws NonFatalException {
		LOG.debug("Mail message: " + message);
		if (message.indexOf(RECIPIENT_MARKER) == -1
				|| message.indexOf(URL_MARKER) == -1
				|| message.indexOf(NAME_MARKER) == -1) {
			throw new InvalidMessageFormatException("Invalid format for "
					+ header.getName() + " mail");
		}
		...
		messageParsedIndex += URL_MARKER.length() + url.length()
				+ ELEMENTS_DELIMITER.length();
		String name = message.substring(messageParsedIndex
				+ NAME_MARKER.length());
		LOG.debug("Name: " + name);

		String loginName = "";
		if (header == EmailHeader.RESET_PASSWORD
				|| header == EmailHeader.REGISTRATION_COMPLETE
				|| header == EmailHeader.VERFICATION) {
			messageParsedIndex += NAME_MARKER.length() + name.length()
					+ ELEMENTS_DELIMITER.length();
			loginName = message.substring(messageParsedIndex
					+ LOGINNAME_MARKER.length());
			LOG.debug("LoginName: " + loginName);
		}
		String profileName = "";
		if (header == EmailHeader.REGISTRATION_COMPLETE
				|| header == EmailHeader.VERFICATION) {
			messageParsedIndex += LOGINNAME_MARKER.length()
					+ loginName.length() + ELEMENTS_DELIMITER.length();
			profileName = message.substring(messageParsedIndex
					+ PROFILENAME_MARKER.length());
	...
	private void parseSubscriptionChargeUnsucessfulMail(String message)
			throws NonFatalException {
		...
		if (message.indexOf(RECIPIENT_MARKER) == -1
			...
			throw new InvalidMessageFormatException(
					"Invalid format for subscription charge unsucessful mail");
		}
		...
		messageParsedIndex += NAME_MARKER.length() + name.length()
				+ ELEMENTS_DELIMITER.length();
		String retryDays = message.substring(messageParsedIndex
				+ RETRYDAYS_MARKER.length());
		LOG.debug("retryDays: " + retryDays);

		LOG.debug("Sending subscription charge unsucessful mail");
		emailServices.sendSubscriptionChargeUnsuccessfulEmail(recipient, name,
				retryDays);
	}
	...
	private void parseRetryChargeMail(String message) throws NonFatalException {
		...
		if (message.indexOf(RECIPIENT_MARKER) == -1
			...
			throw new InvalidMessageFormatException(
					"Invalid format for retry charge mail");
		}
		...
		messageParsedIndex += NAME_MARKER.length() + name.length()
				+ ELEMENTS_DELIMITER.length();
		String loginName = message.substring(messageParsedIndex
				+ LOGINNAME_MARKER.length());
		LOG.debug("LoginName: " + loginName);
		/*
		 * messageParsedIndex += NAME_MARKER.length() + name.length() +
	...
	private void parseMailWithRecipientAndName(String message,
			EmailHeader header) throws NonFatalException {
		LOG.debug("Message for " + header.getName() + ": " + message);
		if (message.indexOf(RECIPIENT_MARKER) == -1
				|| message.indexOf(NAME_MARKER) == -1) {
			throw new InvalidMessageFormatException(
					"Invalid message format for " + header.getName());
		}
	...
	private void parseSurveyCompletionMail(String message)
			throws NonFatalException {
		...
		if (message.indexOf(RECIPIENT_MARKER) == -1
			...
			throw new InvalidMessageFormatException(
					"Invalid format for Survey completion mail");
		}
		...
		messageParsedIndex += NAME_MARKER.length() + name.length()
				+ ELEMENTS_DELIMITER.length();
		String agentName = message.substring(messageParsedIndex
				+ AGENTNAME_MARKER.length());
		LOG.debug("Agent name: " + agentName);

		messageParsedIndex += AGENTNAME_MARKER.length() + name.length()
				+ ELEMENTS_DELIMITER.length();
		String agentEmail = message.substring(messageParsedIndex
				+ AGENTEMAIL_MARKER.length());
		LOG.debug("Agent email: " + agentEmail);

		messageParsedIndex += AGENTEMAIL_MARKER.length() + name.length()
				+ ELEMENTS_DELIMITER.length();
		String agentProfile = message.substring(messageParsedIndex
				+ PROFILENAME_MARKER.length());
		LOG.debug("Agent profile: " + agentProfile);

		LOG.debug("Sending account completion mail");
		emailServices.sendDefaultSurveyCompletionMail(recipient, name, agentName,
				agentEmail, agentProfile , null, -1 );
	}
	...
	private void parseMailWithRecipientAndAgentDetails(String message)
		...
		messageParsedIndex += NAME_MARKER.length() + name.length()
				+ ELEMENTS_DELIMITER.length();
		String agentEmailId = message.substring(messageParsedIndex
				+ AGENTEMAIL_MARKER.length());
		LOG.debug("agentEmailId: " + agentEmailId);

		messageParsedIndex += AGENTNAME_MARKER.length() + agentName.length()
				+ ELEMENTS_DELIMITER.length();
		String link = message.substring(messageParsedIndex
				+ LINK_MARKER.length());
		LOG.debug("link: " + link);

		messageParsedIndex += LINK_MARKER.length() + link.length()
				+ ELEMENTS_DELIMITER.length();
		String agentPhone = message.substring(messageParsedIndex
				+ AGENTPHONE_MARKER.length());
		LOG.debug("agentPhone: " + agentPhone);

		messageParsedIndex += AGENTPHONE_MARKER.length() + agentPhone.length()
				+ ELEMENTS_DELIMITER.length();
		String agentTitle = message.substring(messageParsedIndex
				+ AGENTTITLE_MARKER.length());
		LOG.debug("agentTitle: " + agentTitle);

		messageParsedIndex += AGENTTITLE_MARKER.length() + agentTitle.length()
				+ ELEMENTS_DELIMITER.length();
		String companyName = message.substring(messageParsedIndex
				+ COMPANYNAME_MARKER.length());
	...
	private void parseSurveyCompletionAdminMail(String message)
			throws NonFatalException {
		LOG.debug("Survey completion admin mail message: " + message);
		if (message.indexOf(RECIPIENT_MARKER) == -1
				|| message.indexOf(LOGINNAME_MARKER) == -1
		...
		messageParsedIndex += LOGINNAME_MARKER.length() + loginName.length()
				+ ELEMENTS_DELIMITER.length();
		String surveyDetail = message.substring(messageParsedIndex
				+ SURVEYDETAIL_MARKER.length());
		LOG.debug("surveyDetail: " + surveyDetail);

		messageParsedIndex += SURVEYDETAIL_MARKER.length() + loginName.length()
				+ ELEMENTS_DELIMITER.length();
		String recipientName = message.substring(messageParsedIndex
				+ RECIPIENT_NAME_MARKER.length());
		LOG.debug("recipientName: " + recipientName);

		messageParsedIndex += RECIPIENT_NAME_MARKER.length()
				+ loginName.length() + ELEMENTS_DELIMITER.length();
		String customerName = message.substring(messageParsedIndex
				+ CUSTOMER_NAME_MARKER.length());
		LOG.debug("customerName: " + customerName);

		messageParsedIndex += CUSTOMER_NAME_MARKER.length()
				+ loginName.length() + ELEMENTS_DELIMITER.length();
		String customerRating = message.substring(messageParsedIndex
				+ CUSTOMER_RATING_MARKER.length());
