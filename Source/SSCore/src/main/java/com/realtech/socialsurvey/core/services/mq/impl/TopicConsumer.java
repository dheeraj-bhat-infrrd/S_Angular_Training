...
public class TopicConsumer implements Runnable {
	...
	private static final String AGENTPROFILE_MARKER = "AGENTPROFILE^^";
	...
	private void parseRegistrationMailMessage(String message) throws NonFatalException {
		...
		if (message.indexOf(RECIPIENT_MARKER) == -1 || message.indexOf(URL_MARKER) == -1 || message.indexOf(FIRSTNAME_MARKER) == -1
	...
	private void parseSurveyCompletionMail(String message) throws NonFatalException {
		...
		String agentProfile = message.substring(messageParsedIndex + AGENTPROFILE_MARKER.length());
		LOG.debug("Agent profile url: " + agentProfile);
