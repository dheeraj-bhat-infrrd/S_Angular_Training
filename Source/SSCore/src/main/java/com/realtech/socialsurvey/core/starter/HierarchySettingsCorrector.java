...
public class HierarchySettingsCorrector extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		for (OrganizationUnitSettings companySetting : companySettings) {
			...
			try {
				List<Region> regions = organizationManagementService.getRegionsForCompany(companySetting.getProfileName());
	...
	private void processRegion(OrganizationUnitSettings regionSetting, Region region) {
		...
		if (regionSetting.getContact_details() != null) {
			...
			if (regionSetting.getContact_details().getMail_ids() != null && regionSetting.getContact_details().getMail_ids().getWork() != null) {
		...
		try {
			List<Branch> branches = organizationManagementService.getBranchesByRegionId(region.getRegionId());
	...
	private void processBranch(OrganizationUnitSettings branchSetting, Branch branch) {
		...
		if (branchSetting.getContact_details() != null) {
			...
			if (branchSetting.getContact_details().getMail_ids() != null && branchSetting.getContact_details().getMail_ids().getWork() != null) {
				LOG.debug("Work email id is set");
