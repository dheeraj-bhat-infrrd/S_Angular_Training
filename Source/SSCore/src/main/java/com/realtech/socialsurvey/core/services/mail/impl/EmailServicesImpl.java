...
public class EmailServicesImpl implements EmailServices {
	...
	public static final Logger LOG = LoggerFactory
			.getLogger(EmailServicesImpl.class);
	...
	public void queueRegistrationInviteMail(String url, String recipientMailId,
		...
		LOG.info("Method for queueing registration invite mail called with url : "
				+ url
				+ " firstName :"
				+ firstName
				+ " and lastName : "
		...
		if (url == null || url.isEmpty()) {
			...
			throw new InvalidInputException(
					"Url is empty or null for sending registration invite mail ");
		}
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			LOG.error("Recipient email Id is empty or null for sending registration invite mail ");
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending registration invite mail ");
		}
		if (firstName == null || firstName.isEmpty()) {
			LOG.error("Firstname is empty or null for sending registration invite mail ");
			throw new InvalidInputException(
					"Firstname is empty or null for sending registration invite mail ");
		}
		...
		contentBuilder.append(CommonConstants.RECIPIENT_MARKER).append(
				recipientMailId);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.URL_MARKER).append(url);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.FIRSTNAME_MARKER).append(firstName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.LASTNAME_MARKER).append(lastName);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.REGISTRATION,
	...
	public void sendRegistrationInviteMail(String url, String recipientMailId,
		...
		LOG.info("Method for sending registration invite mail called with url : "
				+ url
				+ " firstName :"
				+ firstName
				+ " and lastName : "
		...
		messageBodyReplacements
				.setFileName(EmailTemplateConstants.EMAIL_TEMPLATES_FOLDER
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				fullName, url, url, url, recipientMailId, appBaseUrl,
				appBaseUrl));

		LOG.debug("Calling email sender to send mail");
		emailSender.sendEmailWithBodyReplacements(emailEntity, subjectFileName,
				messageBodyReplacements);
	...
	public void sendAgentSurveyReminderMail(String recipientMailId,
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				survey.getAgentName(), customerName,
	...
	public void queueResetPasswordEmail(String url, String recipientMailId,
		...
		LOG.info("Method to queue Email to reset the password link with URL : "
				+ url + "\t and Recipients Mail ID : " + recipientMailId);
		if (url == null || url.isEmpty()) {
			LOG.error("URL generated can not be null or empty");
			throw new InvalidInputException(
					"URL generated can not be null or empty");
		}
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			LOG.error("Recipients Email Id can not be null or empty");
			throw new InvalidInputException(
					"Recipients Email Id can not be null or empty");
		}
		if (name == null || name.isEmpty()) {
			LOG.error("Recipients Name can not be null or empty");
			throw new InvalidInputException(
					"Recipients Name can not be null or empty");
		}
		...
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.URL_MARKER).append(url);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.NAME_MARKER).append(name);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.LOGINNAME_MARKER).append(loginName);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.RESET_PASSWORD,
				contentBuilder.toString());
	...
	public void sendResetPasswordEmail(String url, String recipientMailId,
			String name, String loginName) throws InvalidInputException,
			UndeliveredEmailException {
		LOG.info("Method to send Email to reset the password link with URL : "
				+ url + "\t and Recipients Mail ID : " + recipientMailId);
		if (url == null || url.isEmpty()) {
			LOG.error("URL generated can not be null or empty");
			throw new InvalidInputException(
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				name, loginName, url, url, url, appBaseUrl, appBaseUrl));
	...
	public void queueSubscriptionChargeUnsuccessfulEmail(
			...
			throws InvalidInputException {
		LOG.info("Method to send subscription charge unsuccessful mail to : "
				+ name);
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			LOG.error("Recipient email Id is empty or null for sending unsuccessful subscription charge mail ");
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending subscription charge mail ");
		}
		if (name == null || name.isEmpty()) {
			LOG.error("Name is empty or null for sending subscription charge mail ");
			throw new InvalidInputException(
					"Name is empty or null for sending subscription charge mail ");
		}
		...
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.NAME_MARKER).append(name);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.RETRYDAYS_MARKER).append(retryDays);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.SUBSCRIPTION_CHARGE_UNSUCESSFUL,
				contentBuilder.toString());
	...
	public void sendSubscriptionChargeUnsuccessfulEmail(String recipientMailId,
		...
		LOG.debug("Executing sendSubscriptionChargeUnsuccessfulEmail() with parameters : "
				+ recipientMailId + ", " + name + ", " + retryDays);
		EmailEntity emailEntity = prepareEmailEntityForSendingEmail(recipientMailId);
		String subjectFileName = EmailTemplateConstants.EMAIL_TEMPLATES_FOLDER
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				name, retryDays, recipientMailId, appBaseUrl, appBaseUrl));
	...
	public void queueEmailVerificationMail(String url, String recipientMailId,
		...
		LOG.info("Method to queue verification mail called for url : " + url
				+ " recipientMailId : " + recipientMailId);
		if (url == null || url.isEmpty()) {
			throw new InvalidInputException(
					"URL generated can not be null or empty");
		}
		...
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.URL_MARKER).append(url);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.NAME_MARKER).append(recipientName);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.EMAIL_VERFICATION,
				contentBuilder.toString());
	...
	public void sendEmailVerificationMail(String url, String recipientMailId,
			String recipientName) throws InvalidInputException,
			UndeliveredEmailException {
		LOG.info("Method to send verification mail called for url : " + url
				+ " recipientMailId : " + recipientMailId);
		if (url == null || url.isEmpty()) {
			throw new InvalidInputException(
					"URL generated can not be null or empty");
		...
		fileContentReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				recipientName, url, url, url, appBaseUrl, appBaseUrl));
	...
	public void queueVerificationMail(String url, String recipientMailId,
		...
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.LOGINNAME_MARKER).append(loginName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.PROFILENAME_MARKER).append(profileName);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.VERFICATION,
				contentBuilder.toString());
	...
	public void sendVerificationMail(String url, String recipientMailId,
		...
		fileContentReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				recipientName, recipientName, url, url, url, appBaseUrl,
				profileName, appBaseUrl, profileName, loginName, appBaseUrl,
				appBaseUrl));
	...
	public void queueRegistrationCompletionEmail(String url,
		...
		LOG.info("Method to queue Email to complete registration link with URL: "
				+ url + "\t and Recipients Mail ID: " + recipientMailId);
		if (url == null || url.isEmpty()) {
			LOG.error("URL generated can not be null or empty");
			throw new InvalidInputException(
		...
		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.REGISTRATION_COMPLETE,
				contentBuilder.toString());
	...
	public void sendRegistrationCompletionEmail(String url,
			...
			String loginName) throws InvalidInputException,
			UndeliveredEmailException {
		LOG.info("Method to send Email to complete registration link with URL: "
				+ url + "\t and Recipients Mail ID: " + recipientMailId);
		if (url == null || url.isEmpty()) {
			LOG.error("URL generated can not be null or empty");
			throw new InvalidInputException(
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				name, url, url, url, appBaseUrl, profileName, appBaseUrl,
				profileName, loginName, appBaseUrl, appBaseUrl));
		LOG.debug("Calling email sender to send mail");
		emailSender.sendEmailWithBodyReplacements(emailEntity, subjectFileName,
				messageBodyReplacements);
	...
	public void sendFatalExceptionEmail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending fatal exception mail ");
		}
	...
	public void sendEmailSendingFailureMail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sendEmailSendingFailureMail ");
		}
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				displayName, destinationMailId, stackTrace));
	...
	public void queueRetryChargeEmail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending retry charge mail ");
		}
		if (displayName == null || displayName.isEmpty()) {
			LOG.error("displayName parameter is empty or null for sending retry charge mail ");
			throw new InvalidInputException(
					"displayName parameter is empty or null for sending retry charge mail ");
		}
		...
		contentBuilder.append(CommonConstants.RECIPIENT_MARKER).append(
				recipientMailId);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.NAME_MARKER).append(displayName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.LOGINNAME_MARKER).append(loginName);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.RETRY_CHARGE,
				contentBuilder.toString());
	...
	public void sendRetryChargeEmail(String recipientMailId,
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				displayName, recipientMailId, appBaseUrl, appBaseUrl));
	...
	public void queueRetryExhaustedEmail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending retries exhausted mail ");
		}
		if (displayName == null || displayName.isEmpty()) {
			LOG.error("displayName parameter is empty or null for sending retry exhausted mail ");
			throw new InvalidInputException(
					"displayName parameter is empty or null for sending retry exhausted mail ");
		}
		...
		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.RETRY_EXHAUSTED,
				contentBuilder.toString());
		LOG.info("Queued the retry charged mail");
	}
	...
	public void queueAccountDisabledMail(String recipientMailId,
		...
		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.ACCOUNT_DISABLED,
				contentBuilder.toString());
	...
	public void sendAccountDeletionMail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending account deletion mail ");
		}
		if (displayName == null || displayName.isEmpty()) {
			LOG.error("displayName parameter is empty or null for sending account deletion mail ");
			throw new InvalidInputException(
					"displayName parameter is empty or null for sending account deletion mail ");
		}
	...
	public void queueAccountUpgradeMail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending account upgrade mail ");
		}
		if (displayName == null || displayName.isEmpty()) {
			LOG.error("displayName parameter is empty or null for sending account upgrade mail ");
			throw new InvalidInputException(
					"displayName parameter is empty or null for sending account upgrade mail ");
		}
		...
		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.ACCOUNT_UPGRADE,
				contentBuilder.toString());
	...
	public void queueSurveyCompletionMail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending survey completion mail ");
		}
		if (displayName == null || displayName.isEmpty()) {
			LOG.error("displayName parameter is empty or null for sending account upgrade mail ");
			throw new InvalidInputException(
					"displayName parameter is empty or null for sending survey completion mail ");
		}
		...
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.NAME_MARKER).append(displayName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.AGENTNAME_MARKER).append(agentName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.AGENTEMAIL_MARKER).append(agentEmail);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.PROFILENAME_MARKER)
	...
	public void sendSurveyCompletionMail(String recipientMailId,
		...
		EmailEntity emailEntity = prepareEmailEntityForSendingEmail(
				recipientMailId, agentEmail, agentName);
		String subjectFileName = EmailTemplateConstants.EMAIL_TEMPLATES_FOLDER
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				displayName, agentName, agentName, appBaseUrl,
	...
	public void queueSurveyReminderMail(String recipientMailId,
		...
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.AGENTNAME_MARKER).append(agentName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.LINK_MARKER).append(link);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.AGENTPHONE_MARKER).append(agentPhone);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.AGENTTITLE_MARKER).append(agentTitle);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.COMPANYNAME_MARKER).append(companyName);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.SURVEY_REMINDER,
				contentBuilder.toString());
		LOG.info("Queued the survey completion mail");
	}
	...
	public void sendDefaultSurveyReminderMail(String recipientMailId,
		...
		if (firstName == null || firstName.isEmpty()) {
			...
			throw new InvalidInputException(
					"firstName parameter is empty or null for sending survey completion mail ");
		}

		LOG.info("Sending survey reminder email to : " + recipientMailId);
		EmailEntity emailEntity = prepareEmailEntityForSendingEmail(recipientMailId);

		String agentSignature = emailFormatHelper.buildAgentSignature(
				agentName, agentPhone, agentTitle, companyName);
		...
		emailSender.sendEmailWithSubjectAndBodyReplacements(emailEntity,
				subjectReplacements, messageBodyReplacements);
		LOG.info("Successfully sent survey completion mail");
	}
	...
	public void queueSurveyCompletionMailToAdminsAndAgent(String recipientName,
		...
		if (surveyDetail == null || surveyDetail.isEmpty()) {
			...
			throw new InvalidInputException(
					"surveyDetail parameter is empty or null for sending survey completion mail ");
		}
		...
		contentBuilder.append(CommonConstants.RECIPIENT_MARKER).append(
				recipientMailId);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.LOGINNAME_MARKER)
				.append(recipientMailId);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.SURVEYDETAIL_MARKER)
				.append(surveyDetail);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.RECIPIENT_NAME_MARKER)
				.append(recipientName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.CUSTOMER_NAME_MARKER)
				.append(customerName);
		contentBuilder.append(CommonConstants.ELEMENTS_DELIMITER)
				.append(CommonConstants.CUSTOMER_RATING_MARKER).append(rating);

		LOG.debug("queueing content: " + contentBuilder.toString());
		queueProducer.queueEmail(EmailHeader.SURVEY_COMPLETION_ADMIN,
				contentBuilder.toString());
		LOG.info("Queued the survey completion mail");
	}
	...
	@Async
	@Override
	public void sendSurveyCompletionMailToAdminsAndAgent(String recipientName,
			String recipientMailId, String surveyDetail, String customerName,
			String rating) throws InvalidInputException,
			UndeliveredEmailException {
		if (recipientMailId == null || recipientMailId.isEmpty()) {
		...
		subjectReplacements.setReplacementArgs(Arrays.asList(rating,
				customerName));
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				recipientName, recipientMailId, surveyDetail));
	...
	public void sendSocialPostReminderMail(String recipientMailId,
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				displayName, links, "", agentSignature));
	...
	public void sendContactUsMail(String recipientEmailId, String displayName,
		...
		if (recipientEmailId == null || recipientEmailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email id is null or empty!");
		}
		if (displayName == null || displayName.isEmpty()) {
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				displayName, senderName, senderEmailId, message));
	...
	public void sendDefaultSurveyInvitationMail(String recipientMailId,
		...
		LOG.info("Sending survey reminder email to : " + recipientMailId);
		EmailEntity emailEntity = prepareEmailEntityForSendingEmail(
				recipientMailId, agentEmailId, agentName);
		String subjectFileName = EmailTemplateConstants.EMAIL_TEMPLATES_FOLDER
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				firstName, link, link, link, "", agentSignature,
				recipientMailId, companyName, surveyInitiatedOn, agentEmailId,
	...
	public void sendSubscriptionRevisionMail(String recipientMailId,
			...
			UndeliveredEmailException {
		LOG.info("Sending subscription revision mail to " + recipientMailId
				+ " with name " + name);
		if (recipientMailId == null || recipientMailId.isEmpty()) {
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				name, numOfUsers, oldAmount, revisedAmount, recipientMailId,
	...
	public void sendManualRegistrationLink(String recipientId,
			...
			throws InvalidInputException, UndeliveredEmailException {
		LOG.info("Sending manual registration link to " + recipientId
				+ " and name " + firstName);
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				fullName, fullName, link, link, link));
	...
	public void sendDefaultSurveyInvitationMailByCustomer(
			String recipientMailId, String firstName, String agentName,
		...
		messageBodyReplacements
				.setReplacementArgs(Arrays.asList(appLogoUrl, firstName,
						agentName, link, link, link, appBaseUrl, appBaseUrl));
	...
	public void sendDefaultSurveyRestartMail(String recipientMailId,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			...
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending survey restart mail ");
		}
		if (displayName == null || displayName.isEmpty()) {
			LOG.error("displayName parameter is empty or null for sending survey restart mail ");
			throw new InvalidInputException(
					"displayName parameter is empty or null for sending survey restart mail ");
		}
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				displayName, link, link, link, "", agentSignature));
	...
	public void sendSocialConnectMail(String recipientMailId,
		...
		messageBodyReplacements.setReplacementArgs(Arrays.asList(appLogoUrl,
				displayName, account, recipientMailId, appBaseUrl, appBaseUrl));
	...
	public void sendReportAbuseMail(String recipientMailId, String displayName,
		...
		if (recipientMailId == null || recipientMailId.isEmpty()) {
			LOG.error("Recipient email Id is empty or null for sending survey completion mail ");
			throw new InvalidInputException(
					"Recipient email Id is empty or null for sending report abuse mail ");
		}
		if (displayName == null || displayName.isEmpty()) {
			LOG.error("displayName parameter is empty or null for sending account upgrade mail ");
			throw new InvalidInputException(
					"displayName parameter is empty or null for sending report abuse mail ");
		}
		...
		if (reporterName != null && reporterEmail != null) {
			...
			messageBodyReplacements.setReplacementArgs(Arrays.asList(
					appLogoUrl, displayName, agentName, customerName,
					customerEmail, review, reporterName, reporterEmail, reason,
					appBaseUrl));
		} else {
			messageBodyReplacements
					.setFileName(EmailTemplateConstants.EMAIL_TEMPLATES_FOLDER
	...
	public void sendCorruptDataFromCrmNotificationMail(String firstName,
			String lastName, String recipientMailId, String attachmentLocation)
			throws InvalidInputException, UndeliveredEmailException {
	...
	private EmailEntity prepareEmailEntityForSendingEmail(String recipientMailId) {
		LOG.debug("Preparing email entity for registration invitation for recipientMailId "
				+ recipientMailId);
	...
	private EmailEntity prepareEmailEntityForSendingEmail(
			String recipientMailId, String emailId, String name) {
		LOG.debug("Preparing email entity for sending mail to "
				+ recipientMailId + " agent email name: " + emailId + " name: "
				+ name);
		List<String> recipients = new ArrayList<String>();
		recipients.add(recipientMailId);
		...
		emailEntity.setSenderEmailId(emailId.substring(0,
				emailId.indexOf("@") + 1) + defaultEmailDomain);
		emailEntity.setRecipientType(EmailEntity.RECIPIENT_TYPE_TO);
	...
	public void sendManualSurveyReminderMail(
		...
		if (companySettings != null
				...
				&& companySettings.getMail_content()
						.getTake_survey_reminder_mail() != null) {
			MailContent mailContent = companySettings.getMail_content()
					.getTake_survey_reminder_mail();

			String mailBody = emailFormatHelper.replaceEmailBodyWithParams(
					mailContent.getMail_body(), mailContent.getParam_order());
			String agentSignature = emailFormatHelper.buildAgentSignature(
					agentName, agentPhone, agentTitle, companyName);
			DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd");
			String currentYear = String.valueOf(Calendar.getInstance().get(
			...
			mailBody = mailBody.replaceAll("\\[FirstName\\]",
			...
			mailBody = mailBody.replaceAll("\\[AgentSignature\\]",
					agentSignature);
			mailBody = mailBody.replaceAll("\\[RecipientEmail\\]",
					survey.getCustomerEmailId());
			mailBody = mailBody.replaceAll("\\[SenderEmail\\]",
					user.getEmailId());
			mailBody = mailBody.replaceAll("\\[CompanyName\\]", companyName);
			mailBody = mailBody.replaceAll("\\[InitiatedDate\\]",
			...
			if (mailContent.getMail_subject() != null
					&& !mailContent.getMail_subject().isEmpty()) {
				mailSubject = mailContent.getMail_subject();
				mailSubject = mailSubject.replaceAll("\\[AgentName\\]",
						agentName);
			}
