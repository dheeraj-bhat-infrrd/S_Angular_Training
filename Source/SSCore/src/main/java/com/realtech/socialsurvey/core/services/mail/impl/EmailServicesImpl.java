...
public class EmailServicesImpl implements EmailServices
{
	...
	UndeliveredEmailException {
		...
		LOG.info("Sending survey completion email to : " + recipientMailId);
		EmailEntity emailEntity = prepareEmailEntityForSendingEmail(recipientMailId);
		String subjectFileName = EmailTemplateConstants.EMAIL_TEMPLATES_FOLDER + EmailTemplateConstants.SURVEY_REPORT_MAIL_SUBJECT;

		FileContentReplacements messageBodyReplacements = new FileContentReplacements();
		messageBodyReplacements.setFileName(EmailTemplateConstants.EMAIL_TEMPLATES_FOLDER + EmailTemplateConstants.SURVEY_REPORT_MAIL_BODY);
	...
	public void sendManualSurveyReminderMail(OrganizationUnitSettings companySettings, User user, String agentName, String agentPhone, String agentTitle, String companyName, SurveyPreInitiation survey, String surveyLink){
		...
		if ( companySettings != null && companySettings.getMail_content() != null
			...
			try {
				sendSurveyReminderMail( survey.getCustomerEmailId(), mailSubject, mailBody );
			...
			try {
				sendDefaultSurveyReminderMail( survey.getCustomerEmailId(), survey.getCustomerFirstName(),
