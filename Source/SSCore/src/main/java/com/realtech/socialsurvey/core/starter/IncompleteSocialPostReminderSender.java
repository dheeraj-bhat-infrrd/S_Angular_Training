...
public class IncompleteSocialPostReminderSender {
	...
	public static void main(String[] args) {
		...
		long companyId = 1;
		List<SurveyDetails> incompleteSurveyCustomers = surveyHandler.getIncompleteSocialPostCustomersEmail(companyId);
		...
		for (SurveyDetails survey : incompleteSurveyCustomers) {
			...
			try {
				emailServices.sendSocialPostReminderMail(survey.getCustomerEmail(), survey.getCustomerFirstName() + " " + survey.getCustomerLastName(),
						survey.getAgentName());
