...
$(document).on('click', function (e) {
	...
	if ($('#overlay-send-survey').is(':visible')) {
		$('#overlay-send-survey').find('.wc-review-data-row').each(function() {
...
var smsTimeZoneMouseUp = function (e){
	var container = $('#sms-dd-wrapper-tz');
	if (!container.is(e.target) && container.has(e.target).length == 0){
		container.slideToggle(200);
	}
	$(document).unbind("mouseup",smsTimeZoneMouseUp);
};

var replyScoreMouseUp = function (e) {
	var container = $('#st-dd-wrapper-min-reply');
	if (!container.is(e.target) && container.has(e.target).length == 0) {
		container.slideUp(200);
	}
	$(document).unbind("mouseup", replyScoreMouseUp);
}
...
function autoAppendReplyRatingDropdown(ratingId, classes) {
	autoAppendDropdown(ratingId, classes, 5, 0.5);
}
...
$(document).on('click', '#sms-enabled-flag-chk-box', function(e) {
	var isEnabled;
	if ($(this).hasClass('bd-check-img-checked')) {
		$(this).removeClass('bd-check-img-checked');
		isEnabled = true;
		$("#smsFeatureDiv").removeClass("hide");
	} else {
		isEnabled = false;
		$(this).addClass('bd-check-img-checked');
	...
	if(isEnabled == false){
		toggleAutomaticSmsSurveyReminder(false);
	}
	
	updateSettings( "survey_settings.smsSurveyReminderEnabled", isEnabled);
	toggleManualSmsSurveyReminder(isEnabled);
});

$(document).on('click', '#sms-manual-flag-chk-box', function(e) {
	var isAllowed = false;
	if ($(this).hasClass('bd-check-img-checked')) {
		isAllowed = true;
	} 
	toggleManualSmsSurveyReminder(isAllowed);
});

function toggleManualSmsSurveyReminder(isAllowed) {
	
	if(isAllowed) {
		$("#sms-manual-flag-chk-box").removeClass('bd-check-img-checked');
	} else {
	...
	callAjaxPostWithPayloadData("./settings/manualsmssurveyreminder/toggle.do", function(data) {
		$('#overlay-toast').html(data);
		showToast();
	}, payload, true);
}

$(document).on('click', '#sms-automatic-flag-chk-box', function(e) {
	var isAllowed = false;
	if ($(this).hasClass('bd-check-img-checked')) {
		isAllowed = true;
	} 
	toggleAutomaticSmsSurveyReminder(isAllowed);
});

function toggleAutomaticSmsSurveyReminder(isAllowed) {

	if (isAllowed) {
		$("#sms-automatic-flag-chk-box").removeClass('bd-check-img-checked');
	} else {
	...
	callAjaxPostWithPayloadData("./settings/automaticsmssurveyreminder/toggle.do", function(data) {
		$('#overlay-toast').html(data);
		showToast();
	}, payload, true);
}
...
function updateSettings(settingName, settingStatus) {
	...
	callAjaxPostWithPayloadData("./updatesettings.do", function(data) {
		$('#overlay-toast').html(data);
		showToast();
	}, payload, true);
}

//for sms reminder count
$('body').on('blur', '#max-sms-reminder', function() {
	
	if (validateReminderInterval('max-sms-reminder')) {
		if($("#max-sms-reminder").val() > 3){
			$('#overlay-toast').html("Failed: You cann't set max sms reminder count greater than 3.");
			return false;
		}
		
		updateSettings('survey_settings.maxNumberOfSmsSurveyReminders', $("#max-sms-reminder").val());
	}
});

//for sms reminder interval
$('body').on('blur', '#sms-reminder-interval', function() {
	
	if (validateReminderInterval('sms-reminder-interval')) {
		
		updateSettings('survey_settings.smsSurveyReminderInterval', $("#sms-reminder-interval").val());
	}
});

//for sms send time schedule
$(document).on('click','#smsTimeSetButton',function(){
	var startTime = $('#sms-time-window-start').val();
	var endTime = $('#sms-time-window-end').val();
	var timeZone = $('#timezone-list').val();
	
	if(validationTimeWindow(startTime, endTime, timeZone)){
		$('#sms-time-window-error-msg').addClass("hide");
		setSMSTime(startTime, endTime, timeZone);
	}
});

function validationTimeWindow(startTime, endTime, timeZone){
	if(startTime == undefined || startTime == ''){
		$('#overlay-toast').html("Please select start time!");
		showToast();
		return false;
	}
	
	if(endTime == undefined || endTime == ''){
		$('#overlay-toast').html("Please select end time!");
		showToast();
		return false;
	}
	
	var startTimeMinutesOfDay = getMinuteOfDay(startTime);
	var endTimeMinutesOfDay = getMinuteOfDay(endTime);
	
	var timeWindowDurationInMinutes = 0;
	if(startTimeMinutesOfDay > endTimeMinutesOfDay){
		timeWindowDurationInMinutes = (1440 - startTimeMinutesOfDay) + endTimeMinutesOfDay;
	} else {
		timeWindowDurationInMinutes = endTimeMinutesOfDay - startTimeMinutesOfDay;
	}
	if(timeWindowDurationInMinutes < 120){
		$('#sms-time-window-error-msg').html("Please ensure the delivery time window is at least 2 hrs");
		$('#sms-time-window-error-msg').removeClass("hide");
		return false;
	}
...
function setSMSTime( start, end, timeZone ) {
...
$(document).on('click', '#user-edit-btn', function(e) {
	...
	$("#user-edit-save").on('click', function(e) {
		if (validateQuickEditsForm()) {
			...
			$('#overlay-toast').html('Please Enter valid data.');
			showToast();
		}
	});
...
function saveUserDetailsByAdmin() {
	...
	var firstName = $('#selected-user-first-name').val();
	var lastName = $('#selected-user-last-name').val();
	var emailID = $('#selected-user-email').val();
	var title = $('#selected-user-title').val();
	var name = firstName;
	var profileUrl = $('#selected-user-profileurl').val();
	var address1 = $('#selected-user-address1').val();
	var address2 = $('#selected-user-address2').val();
	var country = $('#selected-user-country').val();
	var countryCode = $('#selected-user-country-code').val();
	var city = $('#selected-user-city').val();
	var state = $('#selected-user-state').val();
	var zipcode = $('#selected-user-zip').val();
	var contactNumber = $('#selected-user-phone').val();
	var webUrl = $('#selected-user-website').val();
	var disclaimer = $('#selected-user-disclaimer').val();
	var aboutMe = $('#selected-user-aboutme').val();
	...
	var isAutoPostEnabled = $('#selected-user-autopost-enabled').val();
	var isSSAdmin = $('#v-ed-ss-admin').val();
	var hidePublicPage = $('#v-ed-hide-public-page').val();
	...
	// Getting license authorizations to submit
	/*var licenceList = [];
...
function bindEditUserClick() {
	$('.edit-user').click(function(e) {
		var title1 = $(this).parent().data('title');
		e.stopPropagation();
		$('.v-um-tbl-icn-wraper').hide();
		if ($(this).hasClass('v-tbl-icn-disabled')) {
			return;
		...
		complete: function (data) {
			if (success) {
				...
				if(customerProfileImageUrl != null && customerProfileImageUrl !=undefined && customerProfileImageUrl != ''){
					...
					$('#ss-survey-prof-img-uploaded').css('background-image','url(' + customerProfileImageUrl + ')');

					$('#ss-survey-prof-img-default').addClass('ss-survey-pi-hide');
					$('#ss-survey-prof-img-clicked').addClass('ss-survey-pi-hide');
					$('#ss-survey-prof-img-hover').addClass('ss-survey-pi-hide');
					$('#ss-survey-prof-img-uploaded').removeClass('ss-survey-pi-hide');
					...
					$(document).on('click','#ss-survey-pi-rem-icn',function(e){
						...
						$('#ss-survey-prof-img-uploaded').css('background-image','');

						$('#ss-survey-prof-img-default').removeClass('ss-survey-pi-hide');
						$('#ss-survey-prof-img-clicked').addClass('ss-survey-pi-hide');
						$('#ss-survey-prof-img-hover').addClass('ss-survey-pi-hide');
						$('#ss-survey-prof-img-uploaded').addClass('ss-survey-pi-hide');
...
function callBackOnProfileImageUpload(data) {
	...
	if($('#quick-edits-img-flag').val() == "true"){
		
		$('#overlay-toast').html($('#display-msg-div').text().trim());
		showToast();
		
		var updatedProfImgUrlForQuickEdits = $('#quick-edits-profile-img-url').val();
		if(updatedProfImgUrlForQuickEdits != null && updatedProfImgUrlForQuickEdits != undefined && updatedProfImgUrlForQuickEdits != ''){
			$('#selected-user-prof-img').show();
			$('#qe-default-prof-img').hide();
			$('#selected-user-prof-img').attr('src',updatedProfImgUrlForQuickEdits);
		}else{
			$('#selected-user-prof-img').hide();
			$('#qe-default-prof-img').show();
		}
				
		$('#prof-message-header').html('');
	}else if( $('#new-dash-flag').val() == "true" ){
		$('#overlay-toast').html($('#display-msg-div').text().trim());
		showToast();
...
$(document).on('input', '#wc-review-table-inner[data-role="agent"] input', function() {
	inputEventOnSurveyInvite(this, false);
});

// Send Survey Admin
$(document).on('input', '#wc-review-table-inner[data-role="admin"] input', function() {
	inputEventOnSurveyInvite(this, true);
});

function inputEventOnSurveyInvite(eventTarget, isForAdmin){
	$(eventTarget).removeClass('error-survey');
	var parentDiv = $(eventTarget).closest(".wc-review-tr");
	
	$(eventTarget).closest(".wc-review-data-col").find(".validation").addClass("hidden");
	
	if (parentDiv.is(':last-child')) {
		var surveyRowId = calculateSurveyInviteNextId(parentDiv.attr("id"));
		
		var htmlData = '';
		if(isForAdmin){
			htmlData = createLastSurveyInviteRowForAdmin(surveyRowId);
		} else {
			htmlData = createLastSurveyInviteRowForAgent(surveyRowId);
		}
		
		parentDiv.after(htmlData);
		applyCssEffectsToContactNumber();
		// enable remove button
...
function createLastSurveyInviteRowForAgent(surveyRowId){
	return '<div id="survey-' + surveyRowId + '" class="wc-review-tr wc-review-data-row clearfix">' +
	...
	'<div class="wc-review-data-col wc-review-tc6 survey-contact float-left"><input type="hidden" id="sel-prof-country-code" value="${countryAbbr}"><div class="float-left lp-con-row lp-row clearfix contact-phone"><div class="lp-edit-wrapper clearfix float-left contact-phone-edit">' +
	'<input class="wc-review-input wc-review-contact reg-details reg-details float-left prof-edditable-sin" type="tel" data-phone-number="work" placeholder="Contact Number"></div>' +
	'</div><div class="validation validationcontact float-left hidden"></div></div><div class="wc-review-data-col wc-review-tc4 last float-left"><div class="wc-review-rmv-icn hide"></div></div></div>';
}

function createLastSurveyInviteRowForAdmin(surveyRowId){
	return '<div id="survey-' + surveyRowId + '" class="wc-review-tr wc-review-data-row clearfix">' +
	'<div class="wc-review-data-col wc-review-tc1 survey-user float-left pos-relative"><input data-name="agent-name" class="wc-review-input wc-review-agentname" placeholder="User Name"><div class="validation validationagent hidden"></div></div>' +
	...
	'<div class="wc-review-data-col wc-review-tc6 survey-contact float-left"><input type="hidden" id="sel-prof-country-code" value="${countryAbbr}"><div class="float-left lp-con-row lp-row clearfix contact-phone"><div class="lp-edit-wrapper clearfix float-left contact-phone-edit">' +
	'<input class="wc-review-input wc-review-contact reg-details reg-details float-left prof-edditable-sin" type="tel" data-phone-number="work" placeholder="Contact Number"></div>' +
	'</div><div class="validation validationcontact float-left hidden"></div></div><div class="wc-review-data-col wc-review-tc5 last float-left"><div class="wc-review-rmv-icn hide"></div></div></div>';
}

function calculateSurveyInviteNextId(surveyInviteRowId){
	surveyInviteRowId = surveyInviteRowId.replace(/(\d+)+/g, function(match, number) { return parseInt(number)+1; });
	return surveyInviteRowId;
}

function applyCssEffectsToContactNumber() {
	
	$('input.wc-review-contact:last').intlTelInput({
		utilsScript :window.location.pathname.includes("/rest/survey/showsurveypage")?"../../../resources/js/utils.js": "../resources/js/utils.js"
	});
	$('div.contact-phone-edit:last').css('width', '100%');
	$('div.contact-phone-edit:last').css('margin', '0px');		
	
	$('div.dial-country-code:last').css({
	...
	$('input.reg-details:last').css('height', '40px');
	$('div.contact-phone:last').css('padding-left', '0px');
	$('input.wc-review-contact:last').css('padding-left', '90px');
	$('input.wc-review-contact:last').css('margin-left', '0px');
	
	$('input.wc-review-contact:last').intlTelInput("setCountry", "us");
	$('input.wc-review-contact:last').intlTelInput("setNumber", "");
	
	$('input.wc-review-contact:last').on('keypress', function (e) {
	...
	$('input.wc-review-contact:last').on('paste', function (e) {
		...
		if (regex.test(pastedData)) {
			if( pastedData.length > 10 ) {
				var lastTenDigit = pastedData.length - 10;
				var contactWithoutDialCode = pastedData.substr( lastTenDigit );
				var dialCode = pastedData.substr( 0, lastTenDigit );
				var countryCodeValue = "us";
				if( dialCode != "0" ) {
					for (var i = 0; i < countryList.length; i++) {
			        	var c = countryList[i];
			        	if( c.length > 2 && c[2] == dialCode  ) {
			        		countryCodeValue = c[1];
			        		break;
			        	}
					}
				}
				$(this).intlTelInput("setCountry", countryCodeValue);
				$(this).val(contactWithoutDialCode);
			}
			else {
				$(this).intlTelInput("setCountry", "us");
				$(this).val(pastedData);
			}
			e.preventDefault();
			return true;
...
$(document).on('click', '#wc-send-survey', function () {
	...
	$('#wc-review-table-inner').find(".wc-review-data-row").each(function() {
			...
			var countryCode =$(this).find('.dial-country-code').text();
			var contactNoWitoutCode = $(this).find('input.wc-review-contact').first().val();
			var contactNo = "";
			if( contactNoWitoutCode != '' ) {

				contactNo = countryCode + contactNoWitoutCode;
			}
			...
			if( contactNoWitoutCode != "" ) {
				...
				var count = contactNoWitoutCode.length;
				if ( count < 10 ) {

					$(this).find('.wc-review-contact').addClass("error-survey");
					$(this).find('.validationcontact').html("Enter a valid contact number.").removeClass("hidden");
					$(this).find('.wc-review-contact').siblings().css("display", "block");
					allowrequest = false;
				}
			}
	...
	// check if there is no duplicate contact number entries
	var receiversContactListLength = receiversList.length;
	var surveyContactIndex = [];
	var duplicateContact = false;
	for (var i = 0; i < receiversContactListLength; i++) {
		for (var j = i + 1; j < receiversContactListLength; j++) {
			var flag = (receiversList[i].value.contactNumber != "" && receiversList[j].value.contactNumber != "") ? true : false;
			if (flag && receiversList[i].value.contactNumber == receiversList[j].value.contactNumber && receiversList[i].value.agentEmailId == receiversList[j].value.agentEmailId) {
				surveyContactIndex.push(i);
				surveyContactIndex.push(j);
			}
		}
	}
	if (surveyContactIndex.length != 0) {
		duplicateContact = true;
		for (var k = 0; k < surveyContactIndex.length; k++) {
			$("#" + receiversList[surveyContactIndex[k]].key).find(".wc-review-contact").addClass("error-survey");
			$("#" + receiversList[surveyContactIndex[k]].key).find(".validationcontact").html("Duplicate not allowed.").removeClass("hidden");
		}
	...
	if (allowrequest) {
		$.ajax({
			url : "./getalreadysurveyedemailids.do?source="+sourceValue,
			dataType: "json",
			...
			success : function(data) {
				...
				if (data.length != 0) {
					for (var i = 0; i < data.length; i++) {
						var emailAlreadySurveyed = data[i].emailId;
						var contactNumberAlreadySurveyed = data[i].contactNumber;
						var alreadySurveyedRefId = data[i].refId;
						if(emailAlreadySurveyed){
							$("#" + alreadySurveyedRefId).find(".survey-email").find(':nth-child(1)').addClass("error-survey");
							$("#" + alreadySurveyedRefId).find(".survey-email").find('.validationemail').html("Already surveyed.").removeClass("hidden");
						}
						
						if(contactNumberAlreadySurveyed){
							$("#" + alreadySurveyedRefId).find(".survey-contact").find('.wc-review-contact').addClass("error-survey");
							$("#" + alreadySurveyedRefId).find(".survey-contact").find('.validationcontact').html("Already surveyed.").removeClass("hidden");
						}
						
						if (receiversList[i].value.emailId == data[j]) {
					...
					$.ajax({
						url : "./sendmultiplesurveyinvites.do?source="+sourceValue,
						dataType: "json",
			            contentType: "application/json;charset=utf-8",
						type : "POST",
						data : receiverList,
...
function removeErrorMessagesAndDetermineIfRequiredDataIsPresent() {
	...
	$('#wc-review-table-inner').find(".wc-review-data-row").each(function() {
		$(this).children().each(function() {
			$(this).find('.wc-review-input').removeClass("error-survey");
			$(this).find('.validation').addClass("hidden");
			if (!$(this).hasClass('last')) {
				var input = $(this).children(":input").val();
				if (input != "") {
					end = true;
...
$('body').on('click', '.sms-dd-item', function() {
	var pageHash = window.location.hash;
	if (pageHash.toLowerCase() == "#showcomplaintressettings") {
		$('#comp-rating-post').val($(this).html());
		$('#sms-dd-wrapper-tz').slideToggle(200);
		return;
	}
	$('#sms-tz-list').val($(this).html());
	$('#sms-dd-wrapper-tz').slideToggle(200);
});
...
$(document).on('click', '#wc-send-survey-upload-cancel', function (event) {
	$(".wc-btn-row").show();
	$("#welcome-popup-invite > .welcome-popup-body-wrapper").show();
...
$(document).on('click', '#wc-send-survey-upload-csv', function (event) {
	$(".wc-btn-row").hide();
	$("#welcome-popup-invite > .welcome-popup-body-wrapper").hide();
...
/* Replies Functionality Start */
let deleteReplyEle = '';
let replyCharLimit = 1000;

$(document).on("click", function (e) {
	let ele = e.target;

	if($(ele).closest('.review-reply-main').length || $(ele).closest('.show-reply-input').length)
		return;

	$('.review-reply-main').each(function (i, v ) {
		if($(v).is(':visible')){
			$(v).addClass('hide');
			$(v).closest('.ppl-content.review-height').find('.show-reply-input').removeClass('hide');
		}
	});
});

$(document).on("keypress", ".review-reply-input, .review-reply-edit-text", function (e) {
	let replyText = $(this).hasClass('review-reply-input') ? $(this).val() : $(this).html();

	if (replyText && replyText.length >= replyCharLimit) {
		e.preventDefault();
		$('#overlay-toast').html("Reply cannot exceed more than ".concat(replyCharLimit, " characters"));
		showToast();
	}
});

$(document).on("click", ".show-reply-input", function (e) {
	$('.review-reply-main').each(function (i, v ) {
		if($(v).is(':visible')){
			$(v).addClass('hide');
			$(v).closest('.ppl-content.review-height').find('.show-reply-input').removeClass('hide');
	...
	$(this).closest('.ppl-content.review-height').find('.review-reply-main').removeClass('hide');
	$(this).closest('.ppl-content.review-height').find('.review-reply-main .review-reply-input').focus();
	$(this).addClass('hide');
});

$(document).on("click", ".review-reply-btn", function (e) {
	let replyText = $(this).prev().val();
	let surveyId = $(this).closest('.dsh-review-cont').attr('survey-mongo-id');

	if (!replyText){
		$(this).prev().focus();
		$('#overlay-toast').html('Please enter your reply');
		showToast();
		return;
	}

	let data = {
		surveyMongoId: surveyId,
		replyText: escapeHtmlString(replyText)
	};

	let self = this;
	addUpdateReply("add", data, function (d) {
		let htmlString = '<div class="review-reply-box" data-reply-id="' + d.replyId + '"> \
			<div class="review-reply-box-container"> \
				<span class="review-reply-owner"><b>' + d.replyByName + '</b></span> \
				<span class="review-reply-text"></span> \
		...
		if (!$(self).closest('.ppl-content').find('.review-reply-section').is(":visible")) {
			$(self).closest('.ppl-content').find('.review-reply-section').removeClass('hide');
			$(self).closest('.ppl-content.review-height').find('.review-reply-main').addClass('hide');
		}

		$(self).closest('.review-reply-main').next().find('.review-reply-container').append(htmlString);
		$(self).closest('.review-reply-main').next().find('.review-reply-container .review-reply-text').text(replyText);
		$(self).prev().val('');
	});
});

$(document).on("click", ".edit-reply", function (e) {
	let oldText = $(this).closest('.review-reply-box').find('.review-reply-text').text();
	$(this).closest('.review-reply-box').find('.review-reply-edit-text').val(oldText);

	$('.review-reply-edit-box').addClass("hide");
	$('.manage-review-reply, .review-reply-text').removeClass("hide");

	$(this).closest('.review-reply-box').find('.review-reply-text').addClass("hide");
	$(this).closest('.review-reply-box').find('.review-reply-edit-box').removeClass("hide");
	$(this).closest('.review-reply-box').find('.review-reply-edit-text').focus();
...
$(document).on("click", ".save-reply-btn", function (e) {
	let replyId = $(this).closest('.review-reply-box').attr("data-reply-id");
	let updatedText = $(this).closest('.review-reply-box').find('.review-reply-edit-text').val();
	let surveyId = $(this).closest('.dsh-review-cont').attr('survey-mongo-id');
	...
	let data = {
		surveyMongoId: surveyId,
		replyId: replyId,
		replyText: escapeHtmlString(updatedText)
	};

	let self = this;
	addUpdateReply("update", data, function (d) {
		$('.review-reply-edit-box').addClass("hide");
		$('.manage-review-reply, .review-reply-text').removeClass("hide");
		$(self).closest('.review-reply-box').find('.review-reply-text').text(updatedText);
	});
});
...
$(document).on("click", ".delete-reply", function (e) {
	...
	$('#overlay-continue').html("Delete");
	$('#overlay-cancel').html("Cancel");
	$('#overlay-header').html("Delete Reply");
	$('#overlay-text').html("Are you sure you want to delete this reply ?");
	$('#overlay-continue').attr("onclick", "deleteReply(".concat(JSON.stringify(data), ");"));
});
...
function addUpdateReply(action, data, callback) {
	showOverlay();
	$.ajax({
		url: action === 'add' ? './createreviewreply.do' : './updatereviewreply.do',
		type: 'POST',
		async: true,
		data: data,
		success: function (d, status, xhr) {
			...
			if (xhr.status === 200 && d !== "error") {
				d = JSON.parse(d);
				callback(d);
				let toastText = "Reply ".concat(action === "add" ? "added" : "updated", " successfully");
				$('#overlay-toast').html(toastText);
				showToast();
			}
			else {
				$('#overlay-toast').html('Something went wrong. Please try again');
				showToast();
			}
		},
		error: function (d) {
...
function deleteReply(data) {
	...
	$.ajax({
		...
		success: function (d, status, xhr) {
			hideOverlay();
			if (xhr.status === 200 && d !== "error") {
				if ($(deleteReplyEle).closest('.review-reply-container').children().length === 1) {
					$(deleteReplyEle).closest('.review-reply-section').addClass('hide');
					$(deleteReplyEle).closest('.ppl-content.review-height').find('.show-reply-input').removeClass('hide');
				}

				$(deleteReplyEle).closest('.review-reply-box').remove();
				$('#overlay-toast').html('Reply deleted successfully');
				showToast();
			}
			else {
				$('#overlay-toast').html('Something went wrong. Please try again');
...
$(document).on("click", ".propagate-btn", function (e) {
	let data = {
		allowReply: !$('#allow-reply-chk-box').hasClass('bd-check-img-checked'),
		reviewReplyScore: $("#rating-min-reply").val()
	};

	showOverlay();
	$.ajax({
		url: './propagatesettings.do',
		type: 'POST',
		async: true,
		data: data,
		success: function (d, status, xhr) {
...
$('body').on('click', '.st-dd-item-min-reply', function () {
	$('#rating-min-reply').val($(this).html());
	$('#st-dd-wrapper-min-reply').slideToggle(200);

	$('#ratingcategory').val('rating-min-reply');

	var rating = $('#rating-min-reply').val();
	var ratingParent = $('#rating-min-reply-parent');
	changeRatingPattern(rating, ratingParent);

	updateReviewReplyScore("rating-settings-form");
});

function updateReviewReplyScore(formid) {
	var url = "./updatereviewreplyscore.do";
	callAjaxFormSubmit(url, updateReviewReplyScoreCallBack, formid);
}
function updateReviewReplyScoreCallBack(response) {
...
$('body').on('click', '#allow-reply-chk-box', function () {
	if ($('#allow-reply-chk-box').hasClass('bd-check-img-checked')) {
		$('#allow-reply-chk-box').removeClass('bd-check-img-checked');
		toggleReviewReply(true, '#allow-reply-chk-box');
	} else {
		$('#allow-reply-chk-box').addClass('bd-check-img-checked');
		toggleReviewReply(false, '#allow-reply-chk-box');
	}
});

function toggleReviewReply(isReplyEnabled, disableEle) {
	...
	var payload = {
		"allowReply": isReplyEnabled
	};
	var success = false;
	$.ajax({
		url: "./togglereviewreply.do",
		type: "POST",
		data: payload,
		success: function (data) {
			if (data.errCode == undefined)
		...
		complete: function (data) {
			enable(disableEle);
			if (success) {
				$('#overlay-toast').html("Updated settings");
				showToast();
			}

			if (isReplyEnabled)
				$('.manage-reply-setting-section').find('.st-score-rt-line2').removeClass('disable');
			else
				$('.manage-reply-setting-section').find('.st-score-rt-line2').addClass('disable');
		},
		error: function (e) {
			if (e.status == 504) {
				redirectToLoginPageOnSessionTimeOut(e.status);
			...
			$('#overlay-toast').html("Could not update settings");
			showToast();
		}
	});
}

$('body').on('click', '#allow-reply-all-chk-box', function () {
	if ($('#allow-reply-all-chk-box').hasClass('bd-check-img-checked')) {
		$('#allow-reply-all-chk-box').removeClass('bd-check-img-checked');
		toggleReviewReplyForCompany(true, '#allow-reply-all-chk-box');
	} else {
		$('#allow-reply-all-chk-box').addClass('bd-check-img-checked');
		toggleReviewReplyForCompany(false, '#allow-reply-all-chk-box');
	}
});

function toggleReviewReplyForCompany(isReplyEnabledForCompany, disableEle) {
	...
	var payload = {
		"allowReplyForCompany": isReplyEnabledForCompany
	};
	var success = false;
	$.ajax({
		url: "./togglereviewreplyforcompany.do",
		type: "POST",
		data: payload,
		success: function (data) {
			if (data.errCode == undefined)
...
function updateEventsQuickEditsAddress() {
	var countryCode = $('#selected-user-country-code').val();
	if (countryCode == "US") {
		showStateCityRow('v-ed-state-city-row', 'selected-user-state', 'selected-user-city');
		selectedCountryRegEx = "^" + "\\b\\d{5}\\b(?:[- ]{1}\\d{4})?" + "$";
		selectedCountryRegEx = new RegExp(selectedCountryRegEx);
	} else {
		hideStateCityRow('v-ed-state-city-row', 'selected-user-state');
	}

	attachAutocompleteCountry('selected-user-country', 'selected-user-country-code', 'selected-user-state', 'v-ed-state-city-row', 'selected-user-city',undefined,true);
}

function updateEventsQuickEditsPhone(phoneNumber){
	phoneNumber = phoneNumber.replace(/\(|\)|\-|x| /g,'');
	
	$('#selected-user-phone').intlTelInput({
		utilsScript : "../resources/js/utils.js"
	}).done(initializeWorkPhoneNumberForQuickEdits(phoneNumber));

	$('.dial-country-code').css('font-size', '13px');
	$('.dial-country-code').css('line-height', '39px')
	$('.intl-tel-input').css('float', 'left');
	$('.intl-tel-input.allow-dropdown').find('input').css('padding-left','80px')

	var countryData = $('#selected-user-phone').intlTelInput("getSelectedCountryData");
	$("#selected-user-phone").on("countrychange", function(e, countryData) {
		maskPhoneNumber("selected-user-phone", countryData.iso2);
	});

	initializeWorkPhoneNumberForQuickEdits(phoneNumber);

	if ($('#selected-user-phone').val() == '(') {
		$('#selected-user-phone').val('');
	}
...
function initializeWorkPhoneNumberForQuickEdits(phoneNumber) {
	var countryCode = $('#selected-user-country-code').val();
	if (countryCode == undefined || countryCode == "") {
		countryCode = "us";
	}
	$('#selected-user-phone').intlTelInput("setCountry", countryCode);
	$('#selected-user-phone').intlTelInput("setNumber", phoneNumber);
}

$(document).on('change', '#v-ed-prof-logo', function() {

	if (!logoValidate('#v-ed-prof-logo')) {
		console.log("inside log");
		return false;
	}
	showOverlay();
	...
	formData.append("logo", $(this).prop("files")[0]);
	formData.append("logoFileName", $(this).prop("files")[0].name);
	formData.append("forQuickEdits",true);
	...
	delay(function() {
		callAjaxPOSTWithTextData("./updatelogo.do", function(data) {
			...
			var updatedLogoUrlForQuickEdits = $('#quick-edits-logo-url').val();
			if(updatedLogoUrlForQuickEdits != null && updatedLogoUrlForQuickEdits != undefined && updatedLogoUrlForQuickEdits != ''){
				$('#selected-user-logo').show();
				$('#qe-default-user-logo').hide();
				$('#selected-user-logo').attr('src',updatedLogoUrlForQuickEdits);
			}else{
				$('#selected-user-logo').hide();
				$('#qe-default-user-logo').show();
			}
			adjustImage();
...
function disableQuickEdits(){
	$('form input[data-editable="true"]').prop('readonly', true);
	$('form textarea[data-editable="true"]').prop('readonly', true);
	$('form input[data-locked="true"]').prop('readonly', true);
	$('form textarea[data-locked="true"]').prop('readonly', true);
	
	$('.v-ed-click-item').addClass('v-ed-disable-item');
	$(document).off('click','#selected-user-prof-img-edit');
	$(document).on('click','#selected-user-prof-img-edit',function(e){
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
				
		$(this).addClass('v-ed-disable-item');
			
		if(!$('#btn-save-user-assignment').is(':visible')){
			$('#overlay-toast').html('Please click the edit button!!');
			showToast();
		}
	});
			
	$(document).off('click','#selected-user-prof-img-del');
	$(document).on('click','#selected-user-prof-img-del',function(e){
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
	...
	$(document).off('click','#selected-user-logo-edit');
	$(document).on('click','#selected-user-logo-edit',function(e){
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
		...
		if($('#selected-user-logo-cont').attr('data-locked') != undefined && ($('#selected-user-logo-cont').attr('data-locked') == 'true' )){
			$('#overlay-toast').html('Logo has been locked by your Admin. Contact Admin');
			showToast();
		}else if(!$('#btn-save-user-assignment').is(':visible')){
			$('#overlay-toast').html('Please click the edit button!!');
			showToast();
		}
	});
			
	$(document).off('click','#selected-user-logo-del');
	$(document).on('click','#selected-user-logo-del',function(e){
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
	...
	$(document).off('click','#selected-user-autopost');
	$(document).off('click','.v-ed-checkbox');
	$('#selected-user-state').prop('disabled',true);
	
	initalizeUMEditSection();
}

function enableQuickEdits(){
	$('form input[data-editable="true"]').removeAttr("readonly");
	$('form textarea[data-editable="true"]').removeAttr("readonly");
	$('form input[data-locked="true"]').prop('readonly', true);
	$('form textarea[data-locked="true"]').prop('readonly', true);
	$('.v-ed-click-item').removeClass('v-ed-disable-item');
	$('#selected-user-state').prop('disabled',false);
	...
	$(document).on('click','#selected-user-prof-img-edit' , function(e){
		...
		$('#v-ed-prof-image').trigger('click');
	});

	$(document).off('click','#selected-user-autopost');
	$(document).on('click','#selected-user-autopost', function(e){
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
		
		$('.v-ed-rat-dropdown').slideToggle('fast','swing');
		
		$(document).off('click','.v-ed-rat-drop-item');
		$(document).on('click','.v-ed-rat-drop-item',function(e){
			e.stopPropagation();
			e.stopImmediatePropagation();
			e.preventDefault();
			
			$('#selected-user-autopost').val($(this).html());
			$('.v-ed-rat-dropdown').slideToggle('fast','swing');
			
			var rating = $('#selected-user-autopost').val();
			var ratingParent = $('#v-ed-minrat-star-cont');
			changeRatingPattern(rating, ratingParent , false, "", false, true);
		});
	});

	$(document).off('click','.v-ed-checkbox');
	$(document).on('click','.v-ed-checkbox',function(e){
		if($(this).hasClass('v-ed-checkbox-unchecked')){
			$(this).addClass('v-ed-checkbox-checked');
			$(this).removeClass('v-ed-checkbox-unchecked');
			$(this).siblings('.v-ed-checkbox-inp').val(true);
		}else{
			$(this).removeClass('v-ed-checkbox-checked');
			$(this).addClass('v-ed-checkbox-unchecked');
			$(this).siblings('.v-ed-checkbox-inp').val(false);
		}
	});
	...
	$(document).on('click', '#selected-user-prof-img-del', function(e){
		...
		var payload = {
				"forQuickEdits" : true,
				"userIdForQuickEdits" : $('#selected-userid-hidden').val()
		};
		
		callAjaxPostWithPayloadData( "./removeprofileimage.do", function(data){
				$('#overlay-toast').html($('#display-msg-div').text().trim());
				showToast();
	...
	$('#selected-user-profileurl').off('blur');
	$('#selected-user-profileurl').on('blur',function(e){
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
...
function initializeQuickEditsAutoPostScore(){
	var rating = $('#selected-user-autopost').val();
	var ratingParent = $('#v-ed-minrat-star-cont');
	changeRatingPattern(rating, ratingParent , false, "", false, true);
}

function validateQuickEditsForm(){
	var isValid = true;
	
	if($('#selected-user-first-name').val() == ''){
		$('#selected-user-first-name').parent().siblings('.v-ed-invalid-input').show();
		isValid = false;
	}else{
		$('#selected-user-first-name').parent().siblings('.v-ed-invalid-input').hide();
	}
	
	if(!validateEmailIdForQuickEdits('selected-user-email')){
		$('#selected-user-email').parent().siblings('.v-ed-invalid-input').show();
		isValid = false;
	}else{
	...
	if(!(validatePhoneNumber('selected-user-phone',false,true))){
		...
		$('#selected-user-phone').closest('.v-ed-row').find('.v-ed-invalid-input').hide();
	}
	
	if($('#selected-user-profileurl').val() == ''){
		$('#selected-user-profileurl').parent().siblings('.v-ed-invalid-input').show();
		isValid = false;
	}else{
		$('#selected-user-profileurl').parent().siblings('.v-ed-invalid-input').hide();
	}
	
	return isValid;
}

function validateCountryProfileForQuickEdits() {
	var country = $.trim($('#selected-user-country').val());
	if (country == "") {
		return false;
	} else {
		var countryCode = $.trim($('#selected-user-country-code').val());
		if (countryCode == "") {
			return false;
		} else {
			return true;
...
$('#selected-user-country').on('keypress',function(e){
	$('#selected-user-country-code').val('');
});

$('#selected-user-country').on('blur',function(e){
	if($(this).val() == $(this).attr('data-value')){
		$('#selected-user-country-code').val($('#selected-user-country-code').attr('data-value'));
	}
});

function resetQuickEditsForm(){
	$('form input[data-editable]').each(function(){
		$(this).val($(this).attr('data-value'));
	});
	
	$('form textarea[data-editable]').each(function(){
		$(this).val($(this).attr('data-value'));
	});
	
	$('#selected-user-country-code').val($('#selected-user-country-code').attr('data-value'));
	
	$('#selected-user-state').attr('data-value',$('#selected-user-state').parent().attr('data-value'));
	$('#selected-user-state').val($('#selected-user-state').attr('data-value'));
	
	$('#selected-user-autopost').val($('#selected-user-autopost').attr('data-value'));
	
	var rating = $('#selected-user-autopost').val();
	var ratingParent = $('#v-ed-minrat-star-cont');
	changeRatingPattern(rating, ratingParent , false, "", false, true);
	
	var isAutoPostEnabled = $('#selected-user-autopost-enabled').attr('data-value');
	if(isAutoPostEnabled == 'true'){
		if(!$('#selected-user-autopost-enabled').siblings('.v-ed-checkbox').hasClass('v-ed-checkbox-checked')){
			$('#selected-user-autopost-enabled').siblings('.v-ed-checkbox').removeClass('v-ed-checkbox-unchecked');
			$('#selected-user-autopost-enabled').siblings('.v-ed-checkbox').addClass('v-ed-checkbox-checked');
		}
	}else{
		if(!$('#selected-user-autopost-enabled').siblings('.v-ed-checkbox').hasClass('v-ed-checkbox-unchecked')){
			$('#selected-user-autopost-enabled').siblings('.v-ed-checkbox').addClass('v-ed-checkbox-unchecked');
			$('#selected-user-autopost-enabled').siblings('.v-ed-checkbox').removeClass('v-ed-checkbox-checked');
		}
	}
	
	var hidePublicPage = $('#v-ed-hide-public-page').attr('data-value');
	if(hidePublicPage == 'true'){
		if(!$('#v-ed-hide-public-page').siblings('.v-ed-checkbox').hasClass('v-ed-checkbox-checked')){
			$('#v-ed-hide-public-page').siblings('.v-ed-checkbox').removeClass('v-ed-checkbox-unchecked');
			$('#v-ed-hide-public-page').siblings('.v-ed-checkbox').addClass('v-ed-checkbox-checked');
		}
	}else{
		if(!$('#v-ed-hide-public-page').siblings('.v-ed-checkbox').hasClass('v-ed-checkbox-unchecked')){
			$('#v-ed-hide-public-page').siblings('.v-ed-checkbox').addClass('v-ed-checkbox-unchecked');
...
function validateProfileUrl(){
	...
	$.ajax({
		...
		data : payload,
		async : false,
		success : editProfileUrlForQuickEdits,
		complete: function(){
		},
		error : function(e) {
				if(e.status == 504) {
...
function editProfileUrlForQuickEdits(data) {
	
	if(data.toLowerCase() == 'true'){
		$('#overlay-toast').html('Profile URL already exists. Please try again!!');
		showToast();
		$('#selected-user-profileurl').val($('#originalUrl').attr('data-value'));
	}else{
		callAjaxGET("/showurleditwarning.do", function(data) {
...
function createEditProfileUrlPopupForQuickEdits(header, body) {
	...
	$('#overlay-continue').click(function(e) {
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
...
function removeProfileLogo(userId){
	var url='./removelogoimage.do';
	...
	callAjaxPostWithPayloadData( url, function(data){
		...
		var updatedProfileImage = $('#quick-edits-logo-url').val();
		$('#selected-user-logo').attr('src',updatedProfileImage);
		
	}, payload, true);

}

function initalizeUMEditSection(){

	$(document).off('click','.v-ed-txt-item');
	$(document).on('click','.v-ed-txt-item',function(e){
		if($(this).prop('readonly')){
			if($(this).attr('data-locked') != undefined){
				if($(this).attr('data-locked') == 'true'){
					var variable = $(this).closest('.v-ed-row').find('.v-ed-lbl').html();
					$('#overlay-toast').html(variable + ' has been locked by the Admin. Contact admin!!');
					showToast();
				}else if(!$('#btn-save-user-assignment').is(':visible')){
					$('#overlay-toast').html('Please click on the Edit button!!');
					showToast();
				}
			}else if(!$('#btn-save-user-assignment').is(':visible')){
				$('#overlay-toast').html('Please click on the Edit button!!');
	...
	$(document).off('click','.v-ed-textarea');
	$(document).on('click','.v-ed-textarea',function(e){
		if($(this).prop('readonly')){
			if($(this).attr('data-locked') != undefined){
				if($(this).attr('data-locked') == 'true'){
					var variable = $(this).closest('.v-ed-row').find('.v-ed-lbl').html();
	...
	$(document).off('click','.v-ed-openlink-icn');
	$(document).on('click','.v-ed-openlink-icn',function(e){
		e.stopPropagation();
		e.stopImmediatePropagation();
		e.preventDefault();
		
		var url = '';
		if($(this).attr('id') == 'v-ed-profurl-open'){
			url = $('#v-ed-app-base-url').html();
			url += $('#selected-user-profileurl').val();
		}else{
			url = $(this).siblings('.v-ed-inp-cont').find('.v-ed-txt-item').val();
		}
...
function initializeQuickEditsPage(applicationBaseUrl){
	var domainRegex = /^(http|https):\/\//;
	
	applicationBaseUrl = applicationBaseUrl.replace(domainRegex,'');
...
function showProfileImageForSurvey(profileImage){
	...
	$('#ss-survey-prof-img-uploaded').css('background-image','url(' + profileImage + ')');
...
function bindProfileImageForSurveyEvents(){
	$( '#ss-survey-img-import-wrapper' ).off( 'mouseenter mouseleave' );
	$( '#ss-survey-img-import-wrapper' ).hover(
		function() {
			if(!$('#ss-survey-prof-img-uploaded').hasClass('ss-survey-pi-hide') || !$('#ss-survey-prof-img-clicked').hasClass('ss-survey-pi-hide')){
				return;
			}
			
			$('#ss-survey-prof-img-default').addClass('ss-survey-pi-hide');
			$('#ss-survey-prof-img-clicked').addClass('ss-survey-pi-hide');
			$('#ss-survey-prof-img-hover').removeClass('ss-survey-pi-hide');
		}, function() {
			if(!$('#ss-survey-prof-img-uploaded').hasClass('ss-survey-pi-hide') || !$('#ss-survey-prof-img-clicked').hasClass('ss-survey-pi-hide')){
				return;
			}
	...
	$( '.ss-survey-prof-img-common' ).off( 'mouseenter mouseleave' );
	$( '.ss-survey-prof-img-common' ).hover(
		function() {
			if(!$('#ss-survey-prof-img-uploaded').hasClass('ss-survey-pi-hide') || !$('#ss-survey-prof-img-clicked').hasClass('ss-survey-pi-hide')){
				return;
			}
	...
	$(document).off('click','#ss-survey-prof-img-default');
	$(document).on('click','#ss-survey-prof-img-default',function(e){
		$('#ss-survey-prof-img-default').addClass('ss-survey-pi-hide');
		$('#ss-survey-prof-img-clicked').removeClass('ss-survey-pi-hide');
		$('#ss-survey-prof-img-hover').addClass('ss-survey-pi-hide');
	});
	
	$(document).off('click','#ss-survey-prof-img-hover');
	$(document).on('click','#ss-survey-prof-img-hover',function(e){
		$('#ss-survey-prof-img-default').addClass('ss-survey-pi-hide');
		$('#ss-survey-prof-img-clicked').removeClass('ss-survey-pi-hide');
		$('#ss-survey-prof-img-hover').addClass('ss-survey-pi-hide');
	});
