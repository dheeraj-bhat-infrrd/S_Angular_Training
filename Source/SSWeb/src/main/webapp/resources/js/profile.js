...
function paintProfilePage(result) {
	if (result != undefined && result != "") {
		...
		if (profileLevel != 'COMPANY') {
			...
			var htmlContent = '<a target="_blank" class="brd-crm brd-crm-link" href="' + '/findcompany.do?verticalName=' + profileJson.vertical + '">' + profileJson.vertical + '</a>';
...
function paintBreadCrums(url) {
	...
	callAjaxGETWithTextData(url, function(data) {
		...
		if (jsonData.entity) {
			...
			var htmlContent = '<a target="_blank" class="brd-crm brd-crm-link" href="' + getLocationOrigin() + '/findcompany.do?verticalName=' + entityJson[0].breadCrumbProfile + '">' + entityJson[0].breadCrumbProfile + '</a>';
...
function fetchIndividualsForBranch(branchId) {
	...
	var url = getLocationOrigin() + '/rest/profile/branch/' + branchId + '/individuals?start=' + start + "&rows=" + rows;
		...
		if (result != undefined && result.length > 0) {
			$.each(result, function(i, individual) {
				if (individual.contact_details != undefined) {
					...
					individualsHtml += '		<div class="prf-public-txt bd-hr-txt cursor-pointer individual-link" data-profilename="' + individual.profileName + '">' + individual.contact_details.name + '</div>';
					individualsHtml += '	</div>';
					individualsHtml += '</div>';
				}
			});
...
function fetchIndividualsForRegion(regionId) {
	var start = $('.comp-region[data-regionid="' + regionId + '"]').attr('data-start');
	var rows = $('.comp-region[data-regionid="' + regionId + '"]').attr('data-batch');
	var url = getLocationOrigin() + '/rest/profile/region/' + regionId + '/individuals?start=' + start + "&rows=" + rows;
	$("#regionid-hidden").val(regionId);
		...
		if (result != undefined && result.length > 0) {
			...
			$.each(result, function(i, compIndividual) {
				if (compIndividual.contact_details != undefined) {
					...
					compIndividualsHtml += '		<div class="prf-public-txt bd-hr-txt cursor-pointer individual-link" data-profilename="' + compIndividual.profileName + '">' + compIndividual.contact_details.name + '</div>';
...
function paintReviews(result) {
	...
	$.each(result, function(i, reviewItem) {
		...
		/*
		 * <c:if test="${reviewItem.source =='encompass'}"> <div class='verified-badge verify-image float-right' title='Click here to know more'></div> </c:if> <c:if test="${reviewItem.source =='DOTLOOP'}"> <div class='verified-badge verify-image float-right' title='Click here to know more'></div> </c:if>
		 */

		/*
		 * reviewsHtml += ' </div>';
		 * 
		 * reviewsHtml += ' <div class="ppl-share-wrapper clearfix share-plus-height">'; reviewsHtml += ' <div class="float-left blue-text ppl-share-shr-txt">Share</div>'; reviewsHtml += ' <div class="float-left icn-share icn-plus-open"></div>'; reviewsHtml += ' <div class="float-left clearfix ppl-share-social hide">'; reviewsHtml += ' <span id ="fb_' + i + '"class="float-left ppl-share-icns icn-fb icn-fb-pp" onclick="getImageandCaption(' + i + ');" title="Facebook" data-link="https://www.facebook.com/dialog/feed?' + reviewItem.faceBookShareUrl + '&link=' +profileUrl.replace("localhost","127.0.0.1")+ '&description=' + reviewItem.score.toFixed(scoreFixVal) + '-star response from ' + encodeURIComponent(custDispName) + ' for ' + encodeURIComponent(reviewItem.agentName) + ' at SocialSurvey - ' + encodeURIComponent(reviewItem.review) + ' .&redirect_uri=https://www.facebook.com"></span>'; reviewsHtml += ' <input type="hidden" id="twttxt_' + i + '" class ="twitterText_loop" value ="' + reviewItem.score.toFixed(scoreFixVal) + '-star response from ' + encodeURIComponent(custDispName) + ' for ' + encodeURIComponent(reviewItem.agentName) + ' at SocialSurvey - ' + encodeURIComponent(reviewItem.review) + '"/></input>'; reviewsHtml += ' <span id ="twitt_' + i + '" class="float-left ppl-share-icns icn-twit icn-twit-pp" onclick="twitterFn(' + i + ');" title="Twitter" data-link="https://twitter.com/intent/tweet?text=' + reviewItem.score.toFixed(scoreFixVal) + '-star response from ' + encodeURIComponent(custDispName) + ' for ' + encodeURIComponent(reviewItem.agentName) + ' at SocialSurvey - ' + encodeURIComponent(reviewItem.review) + ' &url='+ profileUrl +'"></span>'; reviewsHtml += ' <span class="float-left ppl-share-icns icn-lin icn-lin-pp" title="LinkedIn" data-link="https://www.linkedin.com/shareArticle?mini=true&url=' + profileUrl + '&title=&summary=' + reviewItem.score.toFixed(scoreFixVal) + '-star response from ' + encodeURIComponent(custDispName) + ' for ' + encodeURIComponent(reviewItem.agentName) +' at SocialSurvey - ' + encodeURIComponent(reviewItem.review) + '&source="></span>'; reviewsHtml += ' <span class="float-left" title="Google+"> <button class="g-interactivepost float-left ppl-share-icns icn-gplus" data-contenturl="' + profileUrl + '" data-clientid="' + reviewItem.googleApi + '"data-cookiepolicy="single_host_origin" data-prefilltext="' + reviewItem.score.toFixed(scoreFixVal) + '-star response from ' + encodeURIComponent(custDispName) + ' for ' + encodeURIComponent(reviewItem.agentName) + ' at SocialSurvey - ' + encodeURIComponent(reviewItem.review) + '" data-calltoactionlabel="USE"'+''+'data-calltoactionurl=" ' + profileUrl + '"> <span class="icon">&nbsp;</span> <span class="label">share</span> </button> </span>'; reviewsHtml += ' </div>'; reviewsHtml += ' <div class="float-right" style="margin: 0 -5px;">'; if(reviewItem.source != "Zillow") reviewsHtml += ' <div class="report-abuse-txt report-txt prof-report-abuse-txt">Report Abuse</div>'; reviewsHtml += ' </div>'; reviewsHtml += ' <div class="float-left icn-share icn-remove icn-rem-size hide"></div>'; reviewsHtml += ' </div>'; reviewsHtml += '</div>';
		 */
		...
		/*
		 * if(reviewItem.summary != null && reviewItem.summary.length > 0){ reviewsHtml += '<div class="ppl-content">'+reviewItem.summary+'</div>'; }
		 */

		/*
		 * if(reviewItem.source == "Zillow") { reviewsHtml += '<br><a class="view-zillow-link" href="'+reviewItem.sourceId+'" target="_blank">View on zillow</a>'; }
		 */
...
/*
 * $(document).on('mouseover','.ppl-review-item ',function(e){ $(this).find('.ppl-share-wrapper').css('visibility','visible'); }); $(document).on('mouseleave','.ppl-review-item ',function(e){ $(this).find('.ppl-share-wrapper').css('visibility','hidden');; });
 */
/*
 * $(document).on('mouseover','.ppl-review-item-last ',function(e){ $(this).find('.ppl-share-wrapper').css('visibility','visible'); }); $(document).on('mouseleave','.ppl-review-item-last',function(e){ $(this).find('.ppl-share-wrapper').css('visibility','hidden'); });
 */
	...
	if (!stopFetchReviewPagination || reviewsNextBatch.length > 0) {
		if (!isNextBatch && reviewsNextBatch != undefined && reviewsNextBatch.length > 0) {
			...
			if (stopFetchReviewPagination)
				return; // Return if pagination is stopped
		...
		if (doFetchZillowReviews && profileJson.socialMediaTokens != undefined && profileJson.socialMediaTokens.zillowToken != undefined)
...
function fetchReviewsBasedOnProfileLevel(profileLevel, currentProfileIden, startIndex, numRows, minScore, isAsync, isNextBatch) {
	...
	url += currentProfileIden + "/reviews?start=" + startIndex + "&numRows=" + numRows + "&sortCriteria=" + reviewsSortBy;
	...
	callAjaxGET(url, function(data) {
		...
		if (responseJson != undefined) {
			...
			if (result != undefined && result.length > 0) {
				...
				if ($("#profile-fetch-info").attr("fetch-all-reviews") == "false" && parseInt($('#prof-hidden-review-count').attr("data-nr-review-count")) > 0) {
...
function fetchReviewsCountBasedOnProfileLevel(profileLevel, iden, callbackFunction, minScore, maxScore, notRecommended) {
...
function paintHiddenReviewsCount(data) {
	...
	if (responseJson != undefined) {
		...
		if (reviewsSizeHtml > 0) {
			...
			$("#prof-hidden-review-count").attr("data-nr-review-count", responseJson.entity).html(reviewsSizeHtml);

			/*
			 * $("#prof-hidden-review-count").click(function(){ $('#prof-review-item').html(''); $(this).hide(); startIndex = 0; $("#profile-fetch-info").attr("fetch-all-reviews", "true"); $(window).scrollTop($('#reviews-container').offset().top); showAllReviews = true; var profileLevel = $("#profile-fetch-info").attr("profile-level"); doFetchZillowReviews = true; fetchReviewsBasedOnProfileLevel(profileLevel, currentProfileIden, startIndex, numOfRows, 0 , true); });
			 */
		}
	}
}
...
$('#prof-posts').scroll(function() {
	...
	if (scrollContainer && $(scrollContainer).is(':visible')) {
		if ((scrollContainer.scrollTop >= ((scrollContainer.scrollHeight) - (scrollContainer.clientHeight / 0.75))) && !isLoaderRunningPublicPosts) {
...
function fetchPublicPostsScroll(isNextBatch) {
	if (!isNextBatch && publicPostsNextBatch.length > 0) {
		...
		if (doStopPublicPostPagination)
			return; // If pagination has stopped return the request
...
function paintPublicPosts(posts) {
	...
	$.each(posts, function(i, post) {
		...
		divToPopulate += '<div class="' + elementClass + '">' + hrefComplet + '<div class="tweet-icn ' + iconClass + ' float-left"></div>' + "</a>" + '<div class="tweet-txt float-left">' + '<div class="tweet-text-main">' + linkify(escapeHtml(post.postText)) + '</div>' + '<div class="tweet-text-link"><em>' + post.postedBy + '</em></div>' + '<div class="tweet-text-time"><em>' + convertUserDateToWeekFormt(new Date(post.timeInMillis)) + '</em></div>' + '	</div>' + '</div>';
	});
