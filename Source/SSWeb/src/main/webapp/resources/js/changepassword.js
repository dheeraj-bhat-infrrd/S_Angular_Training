...
function validateCurrentPassword(elementId) {
	...
	if ($(window).width() < 768) {
		...
		if (oldpassword != "") {
			...
			if (oldpassword.length < minPwdLength
					|| oldpassword.length > maxPwdLength) {
				// $('#' + elementId).parent().next('.cp-err').html('Password must be between 6-15 characters.');
				...
				// $('#' + elementId).parent().next('.cp-err').html('Password must contain one special character.');
				// $('#' + elementId).parent().next('.cp-err').show();
				showError('Password must contain one special character');
				return false;
			}
		} else {
			// $('#' + elementId).parent().next('.cp-err').html('Please enter current password.');
			// $('#' + elementId).parent().next('.cp-err').show();
			showError('Please enter password');
			return false;
		}
...
function validateNewPassword(elementId) {
	...
	if ($(window).width() < 768) {
		...
		if (newpassword != "") {
			...
			// $('#' + elementId).parent().next('.cp-err').html('Please enter new password.');
			// $('#' + elementId).parent().next('.cp-err').show();
			showError('Please enter password');
			return false;
		}
...
function validateConfirmNewPassword(pwdId, confirmPwdId) {
	...
	if ($(window).width() < 768) {
		...
		if ($('#' + confirmPwdId).val() != "") {
			if ($('#' + pwdId).val() != $('#' + confirmPwdId).val()) {
				// $('#' + confirmPwdId).parent().next('.cp-err').html('Passwords do not match.');
			...
			// $('#' + confirmPwdId).parent().next('.cp-err').html('Please enter confirm new password.');
			// $('#' + confirmPwdId).parent().next('.cp-err').show();
			showError('Please enter confirm password');
			return false;
		}
