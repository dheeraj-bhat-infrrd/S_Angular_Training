...
<div class="container bd-hr-container">
	...
	<div class="bd-hr-rt-panel col-lg-9 col-md-9 col-sm-9 col-xs-12">
		<div class="bd-hr-rt-header">
			<i><spring:message code="label.woulduliketoadd.key" /> <strong><spring:message
						code="label.new.key" /></strong></i>
		</div>
		<div class="bd-hr-tabs-wrapper">
			<div class="bd-hr-tabs-header clearfix">
				<div class="bd-hr-tabs-header-item float-left">
					<span id="hr-individual-tab" data-tab="individual"
						class="bd-hdr-span"><spring:message
							code="label.individual.key" /></span>
				</div>
				<c:if test="${isBranchAdditionAllowed}">
					<div class="bd-hr-tabs-header-item float-left">
						<span id="hr-office-tab" data-tab="office" class="bd-hdr-span"><spring:message
								code="label.office.key" /></span>
					</div>
				</c:if>
				<c:if test="${isRegionAdditionAllowed}">
					<div class="bd-hr-tabs-header-item float-left">
						<span id="hr-region-tab" data-tab="region"
							class="bd-hdr-span bd-hdr-active-arr bd-hdr-active"><spring:message
								code="label.region.key" /></span>
					</div>
				</c:if>
		...
		<div id="bd-edit-form-section" class="bd-hr-form-wrapper"></div>

	</div>
</div>
<!-- div to temporarily store message to be displayed  -->
<div id="temp-message" class="hide"></div>
<input class="ignore-clear" type="hidden" name="isUserAuthorized"
	id="is-user-authorized" value="${isUserAuthorized}" />
<input class="ignore-clear" type="hidden" id="profile-name"
	value="${profileName}" />
<input class="ignore-clear" type="hidden" id="account-type"
	value="${user.company.licenseDetails[0].accountsMaster.accountName}" />
<input class="ignore-clear" type="hidden" id="highest-role"
	value="${highestrole}" />
