...
<c:if test ="${not empty users}">
	<c:forEach var="branchUser" items="${users}">
		<tr id="tr-user-${branchUser.userId}" clicked="false" class="v-tbl-row v-tbl-row-sel v-tbl-row-ind sel-r${regionId}-b${branchId}-u${branchUser.userId} user-row-${branchId}" data-userid="${branchUser.userId}">
	           <td class="v-tbl-line">
	           ...
	           <td class="v-tbl-role"><c:if test="${branchUser.isRegionAdmin || branchUser.isBranchAdmin}"><spring:message code="label.admin.key"/></c:if>&nbsp;<c:if test="${branchUser.isAgent}"></c:if><spring:message code="label.user.key"/></td>
	           <td class="v-tbl-btns">
	               <div class="clearfix v-tbl-icn-wraper">
	                   <div class="float-left v-tbl-icn v-icn-close" data-userid="${branchUser.userId}"></div>
