			...
			<c:forEach var="survey" items="${incompleteSurveys}">
				<div class="dash-lp-item clearfix">
					<div class="float-left dash-lp-txt">
						<div>${survey.customerFirstName} ${survey.customerLastName}</div>
					...
					<div data-iden="${survey.surveyPreIntitiationId }" class="float-right dash-icn-close cursor-pointer"
						title="Cancel Survey Reminder"></div>
				</div>
			</c:forEach>
		</c:if>
	</div>
						...
						$('.opensanslight')
								.each(
										function(index, currentElement) {
											var dateSplit = $(this).attr(
													'data-modifiedon').split(
													'-');
											var date = convertTimeStampToLocalTimeStamp(new Date(
													dateSplit[0],
													dateSplit[1] - 1,
													dateSplit[2], dateSplit[3],
													dateSplit[4], dateSplit[5]));
											$(this).html(date);
										});
					});
</script>