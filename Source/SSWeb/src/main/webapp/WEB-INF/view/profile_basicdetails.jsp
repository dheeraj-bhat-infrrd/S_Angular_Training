...
<c:choose>
	<c:when test="${profilemasterid == 4}">
		<c:choose>
			<c:when
				test="${not empty profileSettings.contact_details  && not empty profileSettings.contact_details.industry}">
				<c:set value="${profileSettings.contact_details.industry}"
					var="verticalVal"></c:set>
			</c:when>
