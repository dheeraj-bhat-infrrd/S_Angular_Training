			...
			<div data-firstname="${feedback.customerFirstName}" data-lastname="${feedback.customerLastName}"
				...
				<div class="ppl-share-wrapper clearfix">
					...
					<div class="float-left clearfix ppl-share-social hide" style="display: none;">
						...
						<span id ="fb_${loop.index}"class="float-left ppl-share-icns icn-fb" title="Facebook" onclick ="getImageandCaption(${loop.index})" data-link="https://www.facebook.com/dialog/feed?${feedback.faceBookShareUrl}&link=${feedback.completeProfileUrl}&description=<fmt:formatNumber type="number" pattern="#.#" value="${feedback.score}" />-star response from ${feedback.customerFirstName} ${feedback.customerLastName.substring( 0, 1 ).toUpperCase()} for ${feedback.agentName} at SocialSurvey - ${feedback.review} .&redirect_uri=https://www.facebook.com"></span>
						
						<input type="hidden" id="twttxt_${loop.index}" class ="twitterText_loop" value ="<fmt:formatNumber type="number" pattern="#.#" value="${feedback.score}" />-star response from ${feedback.customerFirstName} ${reviewItem.customerLastName.substring( 0, 1 ).toUpperCase()} for ${feedback.agentName} at SocialSurvey - ${feedback.review}"/>
						<span class="float-left ppl-share-icns icn-twit" id ="twitt_${loop.index}" onclick="twitterFn(${loop.index},this);" data-link="https://twitter.com/intent/tweet?text=<fmt:formatNumber type="number" pattern="#.#" value="${feedback.score}" />-star response from ${feedback.customerFirstName} ${feedback.customerLastName.substring( 0, 1 ).toUpperCase()} for ${feedback.agentName} at SocialSurvey - ${feedback.review}&url=${feedback.completeProfileUrl}"></span>
						 <span
							class="float-left ppl-share-icns icn-lin" title="LinkedIn"
							data-link="https://www.linkedin.com/shareArticle?mini=true&url=${feedback.completeProfileUrl} &title=&summary=<fmt:formatNumber type="number" pattern="#.#" value="${feedback.score}" />-star response from ${feedback.customerFirstName} ${feedback.customerLastName.substring( 0, 1 ).toUpperCase()} for ${feedback.agentName} at SocialSurvey - ${feedback.review} + &source="></span>
                        ...
                        <button 
                            ...
                            data-prefilltext="<fmt:formatNumber type="number" pattern="#.#" value="${feedback.score}" />-star response from ${feedback.customerFirstName} ${feedback.customerLastName.substring( 0, 1 ).toUpperCase()} for ${feedback.agentName} at SocialSurvey - ${feedback.review}"
