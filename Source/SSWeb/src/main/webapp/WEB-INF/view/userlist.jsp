...
<c:set var="accountTypeId"
	value="${user.company.licenseDetails[0].accountsMaster.accountsMasterId}" />
<table class="v-um-tbl">
	<tr class="u-tbl-header">
		...
		<td class="v-tbl-rgn-adm text-center"><c:if
				test="${accountTypeId == 4}">
				<spring:message code="label.region.key" />
				<br />
				<spring:message code="label.admin.key" />
			</c:if></td>
		<td class="v-tbl-of-adm text-center"><c:if
				test="${accountTypeId == 4 || accountTypeId == 3}">
				<spring:message code="label.office.key" />
				<br />
				<spring:message code="label.admin.key" />
			</c:if></td>
		<td class="v-tbl-ln-of text-center"><spring:message
				code="label.individual.key" /></td>
	...
	<c:choose>
		<c:when test="${not empty userslist}">
			<c:forEach var="userfromsearch" items="${userslist}">
				...
				<c:if test="${accountTypeId == 4}">
					...
					<c:if
						test="${userfromsearch.isRegionAdmin != null && userfromsearch.isRegionAdmin}">
				...
				<c:if test="${accountTypeId == 4 || accountTypeId == 3}">
					...
					<c:if
						test="${userfromsearch.isBranchAdmin != null && userfromsearch.isBranchAdmin}">
				...
				<c:if
					test="${userfromsearch.isAgent != null && userfromsearch.isAgent}">
				...
				<c:choose>
					<c:when
						test="${userfromsearch.canEdit && user.userId!=userfromsearch.userId }">
				...
				<tr class="u-tbl-row user-row"
					id="user-row-${userfromsearch.userId}"
					data-editable="${userfromsearch.canEdit}">
					<td class="v-tbl-uname fetch-name"
						data-first-name="${userfromsearch.firstName}"
						data-last-name="${userfromsearch.lastName}"
					...
					<c:choose>
						<c:when test="${userfromsearch.status == 2}">
							<td class="v-tbl-online v-tbl-icn ${userstatustickclass}"
								title="<spring:message code="label.notverified.key" />"></td>
						</c:when>
						<c:otherwise>
							<td class="v-tbl-online v-tbl-icn ${userstatustickclass}"
								title="<spring:message code="label.verified.key" />"></td>
						</c:otherwise>
					</c:choose>
					<td class="v-tbl-rem ${admintcanremoveclass} v-icn-rem-user"
						title="<spring:message code="label.remove.key" />"></td>
					<td
						class="v-tbl-edit ${admincaneditclass} v-icn-edit-user edit-user"
						title="<spring:message code="label.edit.key" />"></td>
				</tr>
				<tr class="u-tbl-row u-tbl-row-sel hide user-assignment-edit-row">
					<td id="user-details-and-assignments-${userfromsearch.userId}"
						class="u-tbl-edit-td user-assignment-edit-div" colspan="9">
		...
		<c:otherwise>
			<tr class="u-tbl-row">
				<spring:message code="label.nousersfound.key" />
			</tr>
		</c:otherwise>
	</c:choose>
</table>