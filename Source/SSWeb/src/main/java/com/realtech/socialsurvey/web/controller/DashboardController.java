...
public class DashboardController {
	...
	public String initDashboardPage(Model model, HttpServletRequest request)
		...
		if (user.getCompany() != null
				&& user.getCompany().getLicenseDetails() != null
				&& !user.getCompany().getLicenseDetails().isEmpty()
				&& user.getCompany().getLicenseDetails().get(0)
						.getAccountsMaster() != null) {
			if (user.getCompany().getLicenseDetails().get(0)
					.getAccountsMaster().getAccountsMasterId() == CommonConstants.ACCOUNTS_MASTER_INDIVIDUAL) {
	...
	public String getProfileDetails(Model model, HttpServletRequest request) {
		...
		try {
			if (columnName.equalsIgnoreCase(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				unitSettings = organizationManagementService
						.getCompanySettings(user);
				if (unitSettings.getContact_details() != null
						&& unitSettings.getContact_details().getName() != null) {
					model.addAttribute("name", unitSettings
							.getContact_details().getName());
				}
				model.addAttribute("title", unitSettings.getContact_details()
						.getTitle());
			} else if (columnName
				...
				unitSettings = organizationManagementService
						.getBranchSettingsDefault(columnValue);
				if (unitSettings.getContact_details() != null
						&& unitSettings.getContact_details().getName() != null) {
					model.addAttribute("name", unitSettings
							.getContact_details().getName());
				...
				unitSettings = userManagementService
						.getUserSettings(columnValue);
				model.addAttribute(
						"name",
						user.getFirstName()
								+ " "
			...
			LOG.error(
					"NonFatalException while fetching profile details. Reason :"
							+ e.getMessage(), e);
		...
		try {
			...
			try {
				surveyDetails = profileManagementService.getReviews(iden, -1,
						-1, startIndex, batchSize, profileLevel, true, null,
						null, "date");
				profileManagementService
						.setAgentProfileUrlForReview(surveyDetails);
	...
	public String getReviewCount(Model model, HttpServletRequest request) {
		...
		try {
			...
			// database.
			reviewCount = profileManagementService.getReviewsCount(iden, -1,
					-1, profileLevel, true);
		} catch (NonFatalException e) {
			LOG.error(
					"Non fatal exception caught in getReviewCount() while fetching reviews count. Nested exception is ",
					e);
	...
	public String getName(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (columnName.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				return new Gson().toJson(user.getFirstName() + " "
						+ user.getLastName());
			} else {
				String columnValue = request.getParameter("columnValue");
				if (columnValue != null && !columnValue.isEmpty()) {
	...
	public String getIncompleteSurveyPopup(Model model,
			HttpServletRequest request) {
		LOG.info("Method to get reviews of company, region, branch, agent getReviews() started.");
		List<SurveyPreInitiation> surveyDetails;
		User user = sessionHelper.getCurrentUser();
	...
	public String cancelMultipleSurveyReminder(Model model,
			HttpServletRequest request) {
		...
		try {
			for (String incompleteSurveyIdStr : surveySetToDeleteArray) {
				try {
					...
					throw new NonFatalException(
							"Number format exception occured while parsing incomplet survey id : "
									+ incompleteSurveyIdStr, e);
				}
			}
			surveyPreInitiationService
					.deleteSurveyReminder(incompleteSurveyIds);
		} catch (NonFatalException nonFatalException) {
			LOG.error("Nonfatal exception occured in method cancelSurveyReminder, reason : "
	...
	public String getIncompleteSurveyCount(Model model,
			HttpServletRequest request) {
		LOG.info("Method to get reviews of company, region, branch, agent getReviews() started.");
		List<SurveyPreInitiation> surveyDetails;
		User user = sessionHelper.getCurrentUser();
	...
	public String redirectToSurveyRequestPage(Model model,
			HttpServletRequest request) {
	...
	private List<SurveyPreInitiation> fetchIncompleteSurveys(
			HttpServletRequest request, User user, boolean realtechAdmin)
	...
	public String getRegionBranchOrAgent(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (searchColumn == null || searchColumn.isEmpty()) {
				...
				throw new InvalidInputException(
						"Invalid value (null/empty) passed for search criteria.");
			}
			...
			if (isRealTechAdmin) {
					...
					if (searchColumn.equalsIgnoreCase("company")) {
						model.addAttribute("results",
								organizationManagementService
										.getCompaniesByName(searchKey));
						return JspResolver.DASHBOARD_SEARCHRESULTS;
					} else {
						result = solrSearchService
								.searchBranchRegionOrAgentByName(searchColumn,
										searchKey, columnName, -1);
					}
				} catch (InvalidInputException e) {
					LOG.error(
				...
				try {
					...
					result = solrSearchService.searchBranchRegionOrAgentByName(
							searchColumn, searchKey, columnName,
							regionOrBranchId);
				} catch (NumberFormatException e) {
					LOG.error(
	...
	public String sendMultipleSurveyReminders(Model model,
			HttpServletRequest request) {
		...
		try {
			for (String incompleteSurveyIdStr : surveysSelectedArray) {
				try {
					long incompleteSurveyId = Long
							.parseLong(incompleteSurveyIdStr);
					SurveyPreInitiation survey = surveyHandler
							.getPreInitiatedSurvey(incompleteSurveyId);
					...
					String surveyLink = "";
					if (survey != null) {
						surveyLink = surveyHandler.getSurveyUrl(agentId,
								customerEmail, surveyHandler.composeLink(
										agentId, customerEmail, custFirstName,
										custLastName));
					}
					...
					if (agentSettings.getContact_details() != null
						...
						agentName = agentSettings.getContact_details()
								.getName();
						agentTitle = agentSettings.getContact_details()
								.getTitle();
					}
					...
					if (enableKafka.equals(CommonConstants.YES)) {
						emailServices.queueSurveyReminderMail(customerEmail,
								custFirstName, agentName, surveyLink,
								agentPhone, agentTitle, companyName);
					} else {
						emailServices
								.sendDefaultSurveyReminderMail(customerEmail,
										custFirstName, agentName, surveyLink,
										agentPhone, agentTitle, companyName);
					}
					...
					throw new NonFatalException(
							"Number format exception occured while parsing incomplete survey id : "
									+ incompleteSurveyIdStr, e);
				}
			}
		} catch (NonFatalException nonFatalException) {
	...
	public void getIncompleteSurveyFile(Model model,
		...
		try {
			...
			if (startDateStr != null) {
				try {
					...
					LOG.error(
							"ParseException caught in getCompleteSurveyFile() while parsing startDate. Nested exception is ",
							e);
				}
			}
			...
			try {
				...
				String fileName = "Incomplete_Survey_" + profileLevel + "_"
						+ iden + EXCEL_FILE_EXTENSION;
				XSSFWorkbook workbook = dashboardService
						.downloadIncompleteSurveyData(surveyDetails, fileName);
				...
				try {
					...
					LOG.error(
							"IOException caught in getIncompleteSurveyFile(). Nested exception is ",
							e);
				} finally {
					try {
			...
			LOG.error(
					"Non fatal exception caught in getReviews() while fetching incomplete reviews file. Nested exception is ",
					e);
		}
		...
		try {
			...
			if (realTechAdmin) {
				...
				if (columnValue != null && !columnValue.isEmpty()) {
					...
					.equals(CommonConstants.PROFILE_LEVEL_COMPANY)) {
				iden = user.getCompany().getCompanyId();
			} else if (profileLevel
					.equals(CommonConstants.PROFILE_LEVEL_INDIVIDUAL)) {
				iden = user.getUserId();
			...
			try {
				surveyDetails = profileManagementService.getReviews(iden, -1,
						...
						null);
				String fileName = "Survey_Results_" + profileLevel + "_" + iden
			...
			if (!surveyRecipients.isEmpty()) {
				for (SurveyRecipient recipient : surveyRecipients) {
					...
					if (surveyHandler.getPreInitiatedSurvey(agentId,
							recipient.getEmailId(), recipient.getFirstname(),
	...
	public String getDashboardButtonsOrder(HttpServletRequest request) {
		...
		try {
			if (columnName.equalsIgnoreCase(CommonConstants.COMPANY_ID_COLUMN)) {
				stages = new ArrayList<>(organizationManagementService
						.getCompanySettings(user).getProfileStages());
			} else if (columnName
					.equalsIgnoreCase(CommonConstants.REGION_ID_COLUMN)) {
				stages = new ArrayList<>(organizationManagementService
						.getRegionSettings(columnValue).getProfileStages());
			} else if (columnName
					.equalsIgnoreCase(CommonConstants.BRANCH_ID_COLUMN)) {
				stages = new ArrayList<>(organizationManagementService
						.getBranchSettingsDefault(columnValue)
					...
					.equalsIgnoreCase(CommonConstants.AGENT_ID_COLUMN)) {
				stages = new ArrayList<>(userManagementService.getUserSettings(
						columnValue).getProfileStages());
			}
		} catch (InvalidInputException | NoRecordsFetchedException e) {
			LOG.error(
					"NonFatalException while fetching badge details. Reason :"
							+ e.getMessage(), e);
		}
	...
	public void restartSurvey(HttpServletRequest request) {
		...
		try {
			if (agentIdStr == null || agentIdStr.isEmpty()) {
				throw new InvalidInputException(
						"Invalid value (Null/Empty) found for agentId.");
			}
			long agentId = Long.parseLong(agentIdStr);
			surveyHandler.changeStatusOfSurvey(agentId, customerEmail,
			...
			LOG.error(
					"NonfatalException caught in makeSurveyEditable(). Nested exception is ",
					e);
		}
	}
	...
	public String reportAbuse(HttpServletRequest request) {
		...
		try {
			...
			try {
				...
				LOG.info(
						"Solr Exception occured while fetching agent name. Nested exception is ",
						e);
				throw e;
			}
			...
			// level admin.
			emailServices.sendReportAbuseMail(applicationAdminEmail,
					applicationAdminName, agentName,
					customerName.replaceAll("null", ""), customerEmail, review,
					reason, null, null);
		} catch (NonFatalException e) {
			LOG.error(
					"NonfatalException caught in reportAbuse(). Nested exception is ",
					e);
			return CommonConstants.ERROR;
		}
	...
	public String fetchSocialLinksInPopup(HttpServletRequest request,
			Model model) {
		LOG.info("Method fetchSocialLinksInPopup() called");
		User user = sessionHelper.getCurrentUser();
		OrganizationUnitSettings unitSettings;
		try {
			unitSettings = userManagementService.getUserSettings(user
					.getUserId());
		} catch (InvalidInputException e) {
			LOG.error(
					"NonFatalException while fetching profile details. Reason :"
		...
		if (tokens != null) {
			if (tokens.getFacebookToken() != null
					&& tokens.getFacebookToken().getFacebookPageLink() != null) {
				model.addAttribute("facebookProfileUrl", tokens
						.getFacebookToken().getFacebookPageLink());
			}
			if (tokens.getGoogleToken() != null
					&& tokens.getGoogleToken().getProfileLink() != null) {
				model.addAttribute("googleProfileUrl", tokens.getGoogleToken()
						.getProfileLink());
			}
			if (tokens.getTwitterToken() != null
					&& tokens.getTwitterToken().getTwitterPageLink() != null) {
				model.addAttribute("twitterProfileUrl", tokens
						.getTwitterToken().getTwitterPageLink());
			}
			if (tokens.getLinkedInToken() != null
					&& tokens.getLinkedInToken().getLinkedInPageLink() != null) {
				model.addAttribute("linkedinProfileUrl", tokens
						.getLinkedInToken().getLinkedInPageLink());
			}
		}
