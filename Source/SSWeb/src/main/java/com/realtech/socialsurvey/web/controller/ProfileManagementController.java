...
public class ProfileManagementController {
	...
	public String showProfileEditPage(Model model, HttpServletRequest request) {
		...
		if (entityIdStr == null || entityIdStr.isEmpty()) {
			...
			try {
				...
				LOG.error(
						"Number format exception occurred while parsing the entity id. Reason :"
								+ e.getMessage(), e);
			}
		}
		...
		try {
			getPrimaryHierarchyByEntity(entityType, entityId);
		}
		...
		LockSettings parentLock = fetchParentLockSettings(model, user,
				accountType, userSettings, branchId, regionId, profilesMaster);
		session.setAttribute(CommonConstants.PARENT_LOCK, parentLock);

		// Setting userSettings in session
		OrganizationUnitSettings profileSettings = fetchUserProfile(model,
				user, accountType, userSettings, branchId, regionId,
				profilesMaster);
		if (entityType.equals(CommonConstants.AGENT_ID_COLUMN)) {
			try {
				profileManagementService.aggregateAgentDetails(user,
						profileSettings, parentLock);
			} catch (InvalidInputException | NoRecordsFetchedException e) {
				LOG.error(
						"InvalidInputException while updating profile. Reason :"
								+ e.getMessage(), e);
	...
	public String fetchVerticalsMaster(Model model, HttpServletRequest request) {
		...
		try {
			try {
				List<VerticalsMaster> verticalsMasterList = organizationManagementService
				...
				throw new NonFatalException(
						"Some error occurred while fetching verticals master",
						e);
			}
	...
	private OrganizationUnitSettings fetchUserProfile(Model model, User user,
			AccountType accountType, UserSettings settings, long branchId,
		...
		try {
			profile = profileManagementService.aggregateUserProfile(user,
					accountType, settings, branchId, regionId, profilesMaster);
		} catch (InvalidInputException | NoRecordsFetchedException e) {
			LOG.error("InvalidInputException while fetching profile. Reason :"
					+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private LockSettings fetchParentLockSettings(Model model, User user,
			AccountType accountType, UserSettings settings, long branchId,
			long regionId, int profilesMaster) {
		LOG.debug("Method fetchParentLockSettings() called from ProfileManagementService");
		LockSettings parentLock = null;
		try {
			parentLock = profileManagementService.aggregateParentLockSettings(
					user, accountType, settings, branchId, regionId,
					profilesMaster);
		} catch (InvalidInputException | NoRecordsFetchedException e) {
			LOG.error("InvalidInputException while fetching profile. Reason :"
					+ e.getMessage(), e);
	...
	public String fetchAddressDetailsEdit(Model model,
	...
	public String updateLockSettings(Model model, HttpServletRequest request) {
		...
		try {
			...
			String entityType = (String) session
					.getAttribute(CommonConstants.ENTITY_TYPE_COLUMN);
			LockSettings parentLock = (LockSettings) session
					.getAttribute(CommonConstants.PARENT_LOCK);
			...
			boolean fieldState = Boolean.parseBoolean(request
					.getParameter("state"));
			if (fieldId == null || fieldId.isEmpty()) {
				throw new InvalidInputException(
						"Name passed can not be null or empty",
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				lockSettings = updateLockSettings(lockSettings, parentLock,
						fieldId, fieldState);
				lockSettings = profileManagementService
						.updateLockSettings(
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in editing LockSettings.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LOCK_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while editing LockSettings. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LOCK_UPDATE_UNSUCCESSFUL,
	...
	// Update lockSettings
	private LockSettings updateLockSettings(LockSettings lockSettings,
			LockSettings parentLock, String fieldId, boolean status) {
	...
	public String updateAboutMe(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (aboutMe == null || aboutMe.isEmpty()) {
				throw new InvalidInputException(
						"About me can not be null or empty",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, contactDetailsSettings);
				...
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.REGION_SETTINGS_COLLECTION,
								regionSettings, contactDetailsSettings);
				...
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.BRANCH_SETTINGS_COLLECTION,
								branchSettings, contactDetailsSettings);
				...
				solrSearchService.editUserInSolr(agentSettings.getIden(),
						CommonConstants.ABOUT_ME_SOLR, aboutMe);
			} else {
				throw new InvalidInputException(
						"Error occurred while updating About me.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.ABOUT_ME_DETAILS_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating about me details. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute(
					"message",
					messageUtils
							.getDisplayMessage(
									DisplayMessageConstants.ABOUT_ME_DETAILS_UPDATE_UNSUCCESSFUL,
									DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String updateBasicDetail(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = updateBasicDetail(
						contactDetailsSettings, name, title, location);
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, contactDetailsSettings);
				...
				// update company name
				profileManagementService.updateCompanyName(user.getUserId(),
						companySettings.getIden(), name);
				if (accountType.getValue() != CommonConstants.ACCOUNTS_MASTER_INDIVIDUAL) {
				...
				// update region name
				profileManagementService.updateRegionName(user.getUserId(),
				...
				// update branch name
				profileManagementService.updateBranchName(user.getUserId(),
				...
				// update user name
				profileManagementService.updateIndividualName(user.getUserId(),
						agentSettings.getIden(), name);
				...
				if (name.indexOf(" ") != -1) {
					userMap.put(CommonConstants.USER_FIRST_NAME_SOLR,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in upadting Basic details.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.BASIC_DETAILS_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating profile basic details. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.BASIC_DETAILS_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private ContactDetailsSettings updateBasicDetail(
			ContactDetailsSettings contactDetailsSettings, String name,
	...
	public String updateProfileAddress(Model model, HttpServletRequest request) {
		...
		try {
			...
			String countryCode = request
					.getParameter(CommonConstants.COUNTRY_CODE);
			String zipcode = request.getParameter(CommonConstants.ZIPCODE);
			if (address1 == null || address1.isEmpty()) {
				throw new InvalidInputException(
						"Address 1 passed can not be null or empty",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			if (country == null || country.isEmpty()) {
				throw new InvalidInputException(
						"country passed can not be null or empty",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			if (zipcode == null || zipcode.isEmpty()) {
				throw new InvalidInputException(
						"zipcode passed can not be null or empty",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = companySettings.getContact_details();
				contactDetailsSettings = updateAddressDetail(
						contactDetailsSettings, address1, address2, country,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in editing Address details.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute(
					...
					messageUtils
							.getDisplayMessage(
									DisplayMessageConstants.PROFILE_ADDRESSES_UPDATE_SUCCESSFUL,
									DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating profile address details. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute(
					"message",
					messageUtils
							.getDisplayMessage(
									DisplayMessageConstants.PROFILE_ADDRESSES_UPDATE_UNSUCCESSFUL,
									DisplayMessageType.ERROR_MESSAGE));
		}
	...
	// Update address details
	private ContactDetailsSettings updateAddressDetail(
			ContactDetailsSettings contactDetailsSettings, String address1,
			String address2, String country, String countryCode, String state,
	...
	public String editCompanyInformation(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (address1 == null || address1.isEmpty()) {
				throw new InvalidInputException(
						"Address is null or empty while adding company information",
						DisplayMessageConstants.INVALID_ADDRESS);
			}

			if (country == null || country.isEmpty()) {
				throw new InvalidInputException(
						"Country is null or empty while adding company information",
						DisplayMessageConstants.INVALID_COUNTRY);
			}

			if (countryCode == null || countryCode.isEmpty()) {
				throw new InvalidInputException(
						"Country code is null or empty while adding company information",
						DisplayMessageConstants.INVALID_COUNTRY);
			}

			if (zipcode == null || zipcode.isEmpty()) {
				throw new InvalidInputException(
						"Zipcode is not valid while adding company information",
						DisplayMessageConstants.INVALID_ZIPCODE);
			}

			if (companyContactNo == null || companyContactNo.isEmpty()) {
				throw new InvalidInputException(
						"Company contact number is not valid while adding company information",
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = companySettings.getContact_details();
				contactDetailsSettings = editCompanyInformation(
						contactDetailsSettings, address1, address2, country,
						state, city, countryCode, zipcode, companyContactNo);
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, contactDetailsSettings);
	...
	// Update address details
	private ContactDetailsSettings editCompanyInformation(
			ContactDetailsSettings contactDetailsSettings, String address1,
			String address2, String country, String state, String city,
			String countryCode, String zipcode, String contactNo) {
		LOG.debug("Method updateAddressDetail() called from ProfileManagementController");
		contactDetailsSettings.setAddress(address1 + ", " + address2);
		contactDetailsSettings.setAddress1(address1);
		contactDetailsSettings.setAddress2(address2);
		...
		if (contactDetailsSettings.getContact_numbers() == null) {
			contactDetailsSettings
					.setContact_numbers(new ContactNumberSettings());
		}
	...
	public String updateSummaryDetail(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = companySettings.getContact_details();
				contactDetailsSettings = updateSummaryDetail(
						contactDetailsSettings, industry, location, aboutme);
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, contactDetailsSettings);
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in editing Summary details.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
	...
	private ContactDetailsSettings updateSummaryDetail(
			ContactDetailsSettings contactDetailsSettings, String industry,
	...
	public String updateLogo(Model model, HttpServletRequest request,
			@RequestParam("logo") MultipartFile fileLocal) {
		...
		try {
			...
			try {
				if (logoFileName == null || logoFileName.isEmpty()) {
					throw new InvalidInputException(
							"Logo passed is null or empty");
				}
				logoUrl = fileUploadService.fileUploadHandler(fileLocal,
						logoFileName);
				logoUrl = amazonEndpoint + CommonConstants.FILE_SEPARATOR
						+ amazonLogoBucket + CommonConstants.FILE_SEPARATOR
						+ logoUrl;
			} catch (NonFatalException e) {
				LOG.error("NonFatalException while uploading Logo. Reason :"
						+ e.getMessage(), e);
				model.addAttribute("message", messageUtils.getDisplayMessage(
						e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
				return JspResolver.MESSAGE_HEADER;
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in uploading logo.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LOGO_UPLOAD_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while uploading logo. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LOGO_UPLOAD_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String updateProfileImage(Model model, HttpServletRequest request) {
		...
		try {
			...
			try {
				int selectedX = Integer.parseInt(request
						.getParameter("selected_x"));
				int selectedY = Integer.parseInt(request
						.getParameter("selected_y"));
				int selectedW = Integer.parseInt(request
						.getParameter("selected_w"));
				int selectedH = Integer.parseInt(request
						.getParameter("selected_h"));
				int resizeWidth = Integer.parseInt(request
						.getParameter("width"));
				...
				// resizing image
				LOG.debug("Dimensions for resizing: resizeWidth: "
						+ resizeWidth + " resizeHeight: " + resizeHeight);
				BufferedImage bufferedImage = ImageIO.read(bis);
				FileOutputStream fileOuputStream = new FileOutputStream(
						filePath);
				ImageIO.write(bufferedImage, CommonConstants.IMAGE_FORMAT_PNG,
						fileOuputStream);
				fileOuputStream.close();
				uploadUtils.resizeImage(filePath, filePath, resizeWidth,
						resizeHeight);

				// cropping image
				LOG.debug("Co-ordinates for cropping: x: " + selectedX + " y: "
						+ selectedY + " h: " + selectedH + " w: " + selectedW);
				BufferedImage resized = ImageIO.read(new File(filePath));
				BufferedImage croppedImage = uploadUtils.cropImage(resized,
						selectedW, selectedH, selectedX, selectedY);
				fileOuputStream = new FileOutputStream(filePath);
				ImageIO.write(croppedImage, CommonConstants.IMAGE_FORMAT_PNG,
						fileOuputStream);
				fileOuputStream.close();

				// uploading image
				File fileLocal = new File(filePath);
				profileImageUrl = fileUploadService.fileUploadHandler(
						fileLocal, imageFileName);
				profileImageUrl = amazonEndpoint
						+ CommonConstants.FILE_SEPARATOR + amazonImageBucket
						+ CommonConstants.FILE_SEPARATOR + profileImageUrl;
			} catch (NonFatalException e) {
				LOG.error(
						"NonFatalException while uploading Profile Image. Reason :"
								+ e.getMessage(), e);
				model.addAttribute("message", messageUtils.getDisplayMessage(
						e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
				return JspResolver.MESSAGE_HEADER;
			} catch (IOException e) {
				LOG.error("IOException while uploading Profile Image. Reason :"
						+ e.getMessage(), e);
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				// Modify Agent details in Solr
				solrSearchService
						.editUserInSolr(agentSettings.getIden(),
								CommonConstants.PROFILE_IMAGE_URL_SOLR,
								profileImageUrl);
			} else {
				throw new InvalidInputException(
						"Invalid input exception occurred while uploading profile image.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.PROFILE_IMAGE_UPLOAD_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while uploading logo. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.PROFILE_IMAGE_UPLOAD_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
		...
		try {
			...
			try {
				...
				if (payload == null || payload.isEmpty()) {
					throw new InvalidInputException(
							"Maild ids passed was null or empty");
				}
				ObjectMapper mapper = new ObjectMapper();
				mailIds = mapper.readValue(
						payload,
						TypeFactory.defaultInstance().constructCollectionType(
								List.class, MiscValues.class));
				...
				try {
					...
					throw new UserAlreadyExistsException(
							"User already exists with emailId : "
									+ primaryMailId);
				} catch (NoRecordsFetchedException e) {
					LOG.debug("User not registerd already with email Id : "
							+ primaryMailId);
				}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = updateMailSettings(
				...
				throw new InvalidInputException(
						"Invalid input exception occurred while updating emailids.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.MAIL_IDS_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (UserAlreadyExistsException userAlreadyExistsException) {
			LOG.error(
					"UserAlreadyExistsException while updating Mail ids. Reason :"
							+ userAlreadyExistsException.getMessage(),
					userAlreadyExistsException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.USERNAME_ALREADY_TAKEN,
					DisplayMessageType.ERROR_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while updating Mail ids. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.MAIL_IDS_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	// send verification links
	private void sendVerificationLinks(ContactDetailsSettings oldSettings,
			List<MiscValues> mailIds, String entityType,
			OrganizationUnitSettings userSettings)
		...
		if (oldSettings == null) {
			throw new InvalidInputException(
					"No contact details object found for user");
		}
		...
		for (MiscValues mailId : mailIds) {
			...
			if (key.equalsIgnoreCase(CommonConstants.EMAIL_TYPE_WORK)) {
				...
				urlParams.put(CommonConstants.EMAIL_TYPE,
						CommonConstants.EMAIL_TYPE_WORK);
				urlParams.put(CommonConstants.ENTITY_ID_COLUMN,
						userSettings.getIden() + "");
				...
				profileManagementService
						.generateVerificationUrl(
								...
								emailId, userSettings.getContact_details()
										.getName());
			}
		}
	...
	// Update mail ids
	private ContactDetailsSettings updateMailSettings(long entityId,
			ContactDetailsSettings contactDetailsSettings,
			List<MiscValues> mailIds, String entityType)
		...
		for (MiscValues mailId : mailIds) {
			...
			if (key.equalsIgnoreCase(CommonConstants.EMAIL_TYPE_WORK)) {
				...
				mailIdSettings.setWorkEmailVerified(false);
					...
					.equalsIgnoreCase(CommonConstants.EMAIL_TYPE_PERSONAL)) {
	...
	public String updatePhoneNumbers(Model model, HttpServletRequest request) {
		...
		try {
			...
			try {
				if (payload == null || payload.isEmpty()) {
					throw new InvalidInputException(
							"Phone numbers passed was null or empty");
				}
				ObjectMapper mapper = new ObjectMapper();
				phoneNumbers = mapper.readValue(
						payload,
						TypeFactory.defaultInstance().constructCollectionType(
								List.class, MiscValues.class));
			} catch (IOException ioException) {
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = companySettings.getContact_details();
				contactDetailsSettings = updatePhoneNumbers(
						contactDetailsSettings, phoneNumbers);
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, contactDetailsSettings);
				...
				throw new InvalidInputException(
						"Invalid input exception occurred while updating phone numbers.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.CONTACT_NUMBERS_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating contact numbers. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute(
					"message",
					messageUtils
							.getDisplayMessage(
									DisplayMessageConstants.CONTACT_NUMBERS_UPDATE_UNSUCCESSFUL,
									DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private ContactDetailsSettings updatePhoneNumbers(
			ContactDetailsSettings contactDetailsSettings,
		...
		ContactNumberSettings phoneNumberSettings = contactDetailsSettings
				.getContact_numbers();
	...
	public String updateWebAddresses(Model model, HttpServletRequest request) {
		...
		try {
			...
			try {
				...
				if (payload == null || payload.isEmpty()) {
					throw new InvalidInputException(
							"Web addresses passed was null or empty");
				}
				ObjectMapper mapper = new ObjectMapper();
				webAddresses = mapper.readValue(
						payload,
						TypeFactory.defaultInstance().constructCollectionType(
								List.class, MiscValues.class));
			} catch (IOException ioException) {
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				contactDetailsSettings = companySettings.getContact_details();
				contactDetailsSettings = updateWebAddresses(
						contactDetailsSettings, webAddresses);
				contactDetailsSettings = profileManagementService
						.updateContactDetails(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, contactDetailsSettings);
				...
				throw new InvalidInputException(
						"Invalid input exception occurred while updating web addresses.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.WEB_ADDRESSES_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating web addresses. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.WEB_ADDRESSES_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private ContactDetailsSettings updateWebAddresses(
			ContactDetailsSettings contactDetailsSettings,
		...
		WebAddressSettings webAddressSettings = contactDetailsSettings
				.getWeb_addresses();
		...
		for (MiscValues webAddress : webAddresses) {
			...
			try {
				...
				throw new InvalidInputException(
						"Web address passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			if (key.equalsIgnoreCase("work")) {
		...
		try {
			...
			try {
				if (fbLink == null || fbLink.isEmpty()) {
					throw new InvalidInputException(
							"Facebook link passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
				urlValidationHelper.validateUrl(fbLink);
			} catch (IOException ioException) {
				throw new InvalidInputException(
						"Facebook link passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				socialMediaTokens = updateFacebookToken(socialMediaTokens,
						fbLink);
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, socialMediaTokens);
				...
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.REGION_SETTINGS_COLLECTION,
								regionSettings, socialMediaTokens);
				...
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.BRANCH_SETTINGS_COLLECTION,
								branchSettings, socialMediaTokens);
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in updating fb token.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.FACEBOOK_TOKEN_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating facebook link in profile. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.FACEBOOK_TOKEN_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateFacebookToken(
			SocialMediaTokens socialMediaTokens, String fbLink) {
		...
		try {
			...
			try {
				if (twitterLink == null || twitterLink.isEmpty()) {
					throw new InvalidInputException(
							"Twitter link passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
				urlValidationHelper.validateUrl(twitterLink);
			} catch (IOException ioException) {
				throw new InvalidInputException(
						"LinkedIn link passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				socialMediaTokens = companySettings.getSocialMediaTokens();
				socialMediaTokens = updateTwitterToken(socialMediaTokens,
						twitterLink);
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in updating twitter token.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.TWITTER_TOKEN_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating twitter link in profile. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.TWITTER_TOKEN_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateTwitterToken(
			SocialMediaTokens socialMediaTokens, String twitterLink) {
		...
		try {
			...
			try {
				if (linkedinLink == null || linkedinLink.isEmpty()) {
					throw new InvalidInputException(
							"LinkedIn link passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				socialMediaTokens = companySettings.getSocialMediaTokens();
				socialMediaTokens = updateLinkedinToken(linkedinLink,
						socialMediaTokens);
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in upadting linkedin token.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LINKEDIN_TOKEN_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating linkedIn link in profile. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LINKEDIN_TOKEN_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateLinkedinToken(String linkedinLink,
		...
		try {
			...
			try {
				if (yelpLink == null || yelpLink.isEmpty()) {
					throw new InvalidInputException(
							"Yelp link passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
				urlValidationHelper.validateUrl(yelpLink);
			} catch (IOException ioException) {
				throw new InvalidInputException("Yelp link passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				profileManagementService
						.updateProfileStages(
								companySettings.getProfileStages(),
								companySettings,
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION);
				companySettings.setSocialMediaTokens(socialMediaTokens);
				userSettings.setCompanySettings(companySettings);
				...
				profileManagementService
						.updateProfileStages(
								regionSettings.getProfileStages(),
								regionSettings,
								MongoOrganizationUnitSettingDaoImpl.REGION_SETTINGS_COLLECTION);
				regionSettings.setSocialMediaTokens(socialMediaTokens);
				userSettings.getRegionSettings().put(entityId, regionSettings);
				...
				profileManagementService
						.updateProfileStages(
								branchSettings.getProfileStages(),
								branchSettings,
								MongoOrganizationUnitSettingDaoImpl.BRANCH_SETTINGS_COLLECTION);
				branchSettings.setSocialMediaTokens(socialMediaTokens);
				userSettings.getRegionSettings().put(entityId, branchSettings);
				...
				profileManagementService
						.updateProfileStages(
								agentSettings.getProfileStages(),
								agentSettings,
								MongoOrganizationUnitSettingDaoImpl.AGENT_SETTINGS_COLLECTION);
				agentSettings.setSocialMediaTokens(socialMediaTokens);
				userSettings.setAgentSettings(agentSettings);
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in updating yelp token.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.YELP_TOKEN_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating yelp link in profile. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.YELP_TOKEN_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateYelpLink(String yelpLink,
			SocialMediaTokens socialMediaTokens) {
		...
		try {
			...
			try {
				if (gplusLink == null || gplusLink.isEmpty()) {
					throw new InvalidInputException(
							"GooglePlus link passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
				urlValidationHelper.validateUrl(gplusLink);
			} catch (IOException ioException) {
				throw new InvalidInputException(
						"GooglePlus link passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				socialMediaTokens = companySettings.getSocialMediaTokens();
				socialMediaTokens = updateGoogleToken(socialMediaTokens,
						gplusLink);
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in updating GooglePlus url",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.GPLUS_TOKEN_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating GooglePlus link in profile. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.GPLUS_TOKEN_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateGoogleToken(
			SocialMediaTokens socialMediaTokens, String gplusLink) {
		...
		try {
			...
			try {
				if (zillowlink == null || zillowlink.isEmpty()) {
					throw new InvalidInputException(
							"Zillow link passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
				urlValidationHelper.validateUrl(zillowlink);
			} catch (IOException ioException) {
				throw new InvalidInputException(
						"Zillow link passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in updating zillow token.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.ZILLOW_TOKEN_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating ZillowLink in profile. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.ZILLOW_TOKEN_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateZillowLink(String zillowLink,
			SocialMediaTokens socialMediaTokens) {
		...
		try {
			...
			try {
				if (realtorLink == null || realtorLink.isEmpty()) {
					throw new InvalidInputException(
							"realtorLink passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
				urlValidationHelper.validateUrl(realtorLink);
			} catch (IOException ioException) {
				throw new InvalidInputException(
						"realtorLink passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				socialMediaTokens = companySettings.getSocialMediaTokens();
				socialMediaTokens = updateRealtorLink(realtorLink,
						socialMediaTokens);
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in updating lendingTree token.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.REALTOR_TOKEN_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating ZillowLink in profile. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.REALTOR_TOKEN_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateRealtorLink(String realtorLink,
			SocialMediaTokens socialMediaTokens) {
		...
		try {
			...
			try {
				if (lendingTreeLink == null || lendingTreeLink.isEmpty()) {
					throw new InvalidInputException(
							"lendingTreeLink passed was null or empty",
							DisplayMessageConstants.GENERAL_ERROR);
				}
				urlValidationHelper.validateUrl(lendingTreeLink);
			} catch (IOException ioException) {
				throw new InvalidInputException(
						"lendingTreeLink passed was invalid",
						DisplayMessageConstants.GENERAL_ERROR, ioException);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				socialMediaTokens = companySettings.getSocialMediaTokens();
				socialMediaTokens = updateLendingTreeLink(lendingTreeLink,
						socialMediaTokens);
				profileManagementService
						.updateSocialMediaTokens(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
			...
			model.addAttribute(
					...
					messageUtils
							.getDisplayMessage(
									DisplayMessageConstants.LENDINGTREE_TOKEN_UPDATE_SUCCESSFUL,
									DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating ZillowLink in profile. Reason :"
			...
			model.addAttribute(
					...
					messageUtils
							.getDisplayMessage(
									DisplayMessageConstants.LENDINGTREE_TOKEN_UPDATE_UNSUCCESSFUL,
									DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private SocialMediaTokens updateLendingTreeLink(String lendingTreeLink,
			SocialMediaTokens socialMediaTokens) {
		...
		LendingTreeToken lendingTreeToken = socialMediaTokens
				.getLendingTreeToken();
	...
	public String updateAchievements(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				achievements = profileManagementService
						.addAchievements(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in adding achievements.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.ACHIEVEMENT_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating achievement details. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.ACHIEVEMENT_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String updateAssociations(Model model, HttpServletRequest request) {
		...
		try {
			...
			try {
				if (payload == null) {
					throw new InvalidInputException(
							"Association passed was null or empty");
				}
				ObjectMapper mapper = new ObjectMapper();
				associations = mapper.readValue(
						payload,
						TypeFactory.defaultInstance().constructCollectionType(
								List.class, Association.class));
			} catch (IOException ioException) {
				throw new NonFatalException(
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				associations = profileManagementService
						.addAssociations(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in adding associations.",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.ASSOCIATION_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating licence details. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.ASSOCIATION_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String updateProfileLicenses(Model model, HttpServletRequest request) {
		...
		try {
			...
			try {
				if (payload == null) {
					throw new InvalidInputException(
							"Licenses passed was null or empty");
				}
				ObjectMapper mapper = new ObjectMapper();
				authorisedIn = mapper.readValue(
						payload,
						TypeFactory.defaultInstance().constructCollectionType(
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				licenses = profileManagementService
						.addLicences(
								MongoOrganizationUnitSettingDaoImpl.COMPANY_SETTINGS_COLLECTION,
								companySettings, authorisedIn);
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LICENSES_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating licence details. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.LICENSES_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String updateExpertise(Model model, HttpServletRequest request) {
		...
		try {
			...
			try {
				if (payload == null) {
					throw new InvalidInputException(
							"Expertise passed was null or empty");
				}
				ObjectMapper mapper = new ObjectMapper();
				expertiseList = mapper.readValue(
						payload,
						TypeFactory.defaultInstance().constructCollectionType(
								List.class, String.class));
			} catch (IOException ioException) {
			...
			if (entityType.equals(CommonConstants.AGENT_ID_COLUMN)) {
				...
				profileManagementService.updateAgentExpertise(agentSettings,
						expertiseList);
				agentSettings.setExpertise(expertiseList);
				userSettings.setAgentSettings(agentSettings);
			} else {
				throw new InvalidInputException(
						"Invalid input exception occurred in adding expertise.",
						DisplayMessageConstants.GENERAL_ERROR);
			}

			LOG.info("Expertise list updated successfully");
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.EXPERTISE_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while updating expertise. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.EXPERTISE_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String updateHobbies(Model model, HttpServletRequest request) {
		...
		try {
			...
			try {
				if (payload == null) {
					throw new InvalidInputException(
							"Hobbies passed was null or empty");
				}
				ObjectMapper mapper = new ObjectMapper();
				hobbiesList = mapper.readValue(
						payload,
						TypeFactory.defaultInstance().constructCollectionType(
								List.class, String.class));
			} catch (IOException ioException) {
			...
			if (entityType.equals(CommonConstants.AGENT_ID_COLUMN)) {
				...
				profileManagementService.updateAgentHobbies(agentSettings,
						hobbiesList);
				for (ProfileStage stage : agentSettings.getProfileStages()) {
					if (stage.getProfileStageKey().equalsIgnoreCase(
							"HOBBIES_PRF")) {
				...
				throw new InvalidInputException(
						"Invalid input exception occurred in adding hobbies.",
						DisplayMessageConstants.GENERAL_ERROR);
			}

			LOG.info("Hobbies list updated successfully");
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.HOBBIES_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while updating hobbies. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.HOBBIES_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
			...
			try {
				if (patternFirst == null && patternLast == null) {
					...
					throw new InvalidInputException(
							messageUtils
									.getDisplayMessage(
											DisplayMessageConstants.INVALID_FIRSTORLAST_NAME_PATTERN,
											DisplayMessageType.ERROR_MESSAGE)
									.getMessage());
				}
				if (!patternFirst.trim().matches(
						CommonConstants.FINDAPRO_FIRST_NAME_REGEX)
						&& !patternLast.trim().matches(
								CommonConstants.FINDAPRO_LAST_NAME_REGEX)) {
					LOG.error("Invalid search key passed in method findAProfileScroll().");
					throw new InvalidInputException(
							messageUtils
									.getDisplayMessage(
				...
				try {
					SolrDocumentList results = solrSearchService
							.searchUsersByFirstOrLastName(patternFirst.trim(),
									patternLast.trim(), startIndex, batchSize);
					for (SolrDocument solrDocument : results) {
						userIds.add((Long) solrDocument.getFieldValue("userId"));
					}
					users = userManagementService
							.getMultipleUsersByUserId(userIds);
					...
					LOG.error(
							"Error occured while searching in findAProfileScroll(). Reason is ",
							e);
					throw new NonFatalException(
							"Error occured while searching in findAProfileScroll()",
							e);
				}
			} catch (NonFatalException nonFatalException) {
				LOG.error(
						"NonFatalException while searching in findAProfileScroll(). Reason : "
		...
		try {
			...
			try {
				startIndex = Integer.parseInt(request
						.getParameter("find-pro-start-index"));
			} catch (NumberFormatException e) {
			...
			int batchSize;
			try {
				batchSize = Integer.parseInt(request
						.getParameter("find-pro-row-size"));
			} catch (NumberFormatException e) {
	...
	public String initProListByProfileLevelPage(Model model,
			HttpServletRequest request,
		...
		LOG.info("Method initProListByProfileLevelPage called for profileLevel:"
				+ profileLevel
				+ " and iden:"
				+ iden
				+ " and searchCriteria:"
		...
		try {
			if (profileLevel == null || profileLevel.isEmpty()) {
				throw new InvalidInputException(
						"profile level is invalid in initProListByProfileLevelPage",
						DisplayMessageConstants.INVALID_PROFILE_LEVEL);
			}
			if (iden == null || iden <= 0l) {
				throw new InvalidInputException(
						"iden is invalid in initProListByProfileLevelPage",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			LOG.error(
					"NonFatalException in initProListByProfileLevelPage.Reason:"
							+ e.getMessage(), e);
			message = messageUtils.getDisplayMessage(e.getErrorCode(),
					DisplayMessageType.ERROR_MESSAGE);
		...
		if (isBotRequest) {
			...
			try {
				try {
					...
					SolrDocumentList results = solrSearchService
							.getUserIdsByIden(iden, idenFieldName, true, start,
									numRows);
					for (SolrDocument solrDocument : results) {
						userIds.add((Long) solrDocument.getFieldValue("userId"));
					}
					...
					throw new InternalServerException(
							new ProfileServiceErrorCode(
									CommonConstants.ERROR_CODE_PRO_LIST_FETCH_FAILURE,
									CommonConstants.SERVICE_CODE_PRO_LIST_FETCH,
	...
	public String findCompanies(Model model, HttpServletRequest request) {
		...
		try {
			...
			List<OrganizationUnitSettings> companyList = profileManagementService
					.getCompanyList(verticalName);
			...
			LOG.error("NonFatalException caught in findCompanies() method, reason : "
					+ e.getMessage());
		}
	...
	public String getAdminHierarchy(Model model, HttpServletRequest request)
		...
		try {
			String entityType = (String) request.getSession(false)
					.getAttribute(CommonConstants.ENTITY_TYPE_COLUMN);
			if (entityType == null) {
				throw new InvalidInputException(
						"No user settings found in session",
			...
			LOG.error(
					"InvalidInputException while fetching hierarchy. Reason: "
							+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private Model getCompanyHierarchy(Model model, HttpServletRequest request)
		...
		if (companyProfileName == "") {
			...
			throw new InvalidInputException(
					"Invalid companyProfileName passed in method getCompanyHierarchy().");
		}

		// Fetching Regions under Company
		regions = organizationManagementService
				.getRegionsForCompany(companyProfileName);
		model.addAttribute("regions", regions);

		// Fetching Branches under Company
		try {
			branches = organizationManagementService
					.getBranchesUnderCompany(companyProfileName);
			model.addAttribute("branches", branches);
		} catch (NoRecordsFetchedException e) {
			LOG.error(
					"NoRecordsFetchedException while fetching company hierarchy branches. Reason: "
							+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
		...
		try {
			individuals = profileManagementService
					.getIndividualsForCompany(companyProfileName);
			model.addAttribute("individuals", individuals);
		} catch (NoRecordsFetchedException e) {
			LOG.error(
					"NoRecordsFetchedException while fetching company hierarchy individuals. Reason: "
							+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String getCompanyHierarchyDirect(Model model,
			HttpServletRequest request) throws ProfileNotFoundException {
		LOG.info("Method getCompanyHierarchyDirect() called from ProfileManagementController");
		try {
			model = getCompanyHierarchy(model, request);
		} catch (InvalidInputException e) {
			LOG.error(
					"InvalidInputException while fetching company hierarchy. Reason: "
							+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String getRegionHierarchyOnClick(Model model,
			HttpServletRequest request) {
		...
		try {
			...
			LOG.error(
					"InvalidInputException while fetching region hierarchy. Reason: "
							+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private Model getRegionHierarchy(Model model, HttpServletRequest request)
			throws InvalidInputException {
		...
		try {
			...
			LOG.error("NumberFormatException while parsing regionId in getRegionHierarchy.Reason :"
					+ e.getMessage());
			throw new InvalidInputException(
					"Invalid regionId passed in method getRegionHierarchy().");
		}
		if (regionId == 0l) {
			throw new InvalidInputException(
					"Invalid regionId passed in method getRegionHierarchy().",
		...
		branches = organizationManagementService
				.getBranchesByRegionId(regionId);
		model.addAttribute("branches", branches);

		// Fetching Individuals under Region
		try {
			individuals = profileManagementService
					.getIndividualsByRegionId(regionId);
			model.addAttribute("individuals", individuals);
		} catch (NoRecordsFetchedException e) {
			LOG.error(
					"NoRecordsFetchedException while fetching region hierarchy. Reason: "
							+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String getBranchHierarchyOnClick(Model model,
			HttpServletRequest request) {
		...
		try {
			...
			LOG.error(
					"InvalidInputException while fetching branch hierarchy. Reason: "
							+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					e.getErrorCode(), DisplayMessageType.ERROR_MESSAGE));
		}
	...
	private Model getBranchHierarchy(Model model, HttpServletRequest request)
			throws InvalidInputException {
		...
		if (branchId == 0l) {
			...
			throw new InvalidInputException(
					"Invalid branchId passed in method getBranchHierarchy().");
		}

		// Fetching Individuals under Branch
		individuals = profileManagementService
				.getIndividualsByBranchId(branchId);
		model.addAttribute("individuals", individuals);
	...
	public String fetchReviews(Model model, HttpServletRequest request) {
		...
		try {
			...
			double minScore = Double.parseDouble(request
					.getParameter("minScore"));
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				reviewItems = profileManagementService.getReviews(user
						.getCompany().getCompanyId(), minScore, maxScore,
						startIndex, numRows,
						CommonConstants.PROFILE_LEVEL_COMPANY, fetchAbusive,
						null, null, null);
			} else if (entityType.equals(CommonConstants.REGION_ID_COLUMN)) {
				reviewItems = profileManagementService.getReviews(entityId,
						minScore, maxScore, startIndex, numRows,
				...
				reviewItems = profileManagementService.getReviews(
						user.getUserId(), minScore, maxScore, startIndex,
						numRows, CommonConstants.PROFILE_LEVEL_INDIVIDUAL,
						fetchAbusive, null, null, null);
			}
			...
			throw new InternalServerException(new ProfileServiceErrorCode(
					CommonConstants.ERROR_CODE_COMPANY_REVIEWS_FETCH_FAILURE,
					CommonConstants.SERVICE_CODE_COMPANY_REVIEWS,
		...
		try {
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				if (companyId == 0l) {
					...
					throw new InvalidInputException(
							"Invalid companyId passed in method fetchReviews().");
				}

				reviewCount = profileManagementService.getReviewsCount(
						companyId, minScore, maxScore,
						CommonConstants.PROFILE_LEVEL_COMPANY, fetchAbusive);
			} else if (entityType.equals(CommonConstants.REGION_ID_COLUMN)) {
				long regionId = Long
						.parseLong(request.getParameter("regionId"));
				if (regionId == 0l) {
					LOG.error("Invalid regionId passed in method fetchReviews().");
					throw new InvalidInputException(
							"Invalid regionId passed in method fetchReviews().");
				}

				reviewCount = profileManagementService.getReviewsCount(
						regionId, minScore, maxScore,
						CommonConstants.PROFILE_LEVEL_REGION, fetchAbusive);
			} else if (entityType.equals(CommonConstants.BRANCH_ID_COLUMN)) {
				long branchId = Long
						.parseLong(request.getParameter("branchId"));
				if (branchId == 0l) {
					LOG.error("Invalid branchId passed in method fetchReviews().");
					throw new InvalidInputException(
							"Invalid branchId passed in method fetchReviews().");
				}

				reviewCount = profileManagementService.getReviewsCount(
						branchId, minScore, maxScore,
						CommonConstants.PROFILE_LEVEL_BRANCH, fetchAbusive);
			} else if (entityType.equals(CommonConstants.AGENT_ID_COLUMN)) {
				...
				if (agentId == 0l) {
					...
					throw new InvalidInputException(
							"Invalid agentId passed in method fetchReviews().");
				}

				reviewCount = profileManagementService.getReviewsCount(agentId,
						minScore, maxScore,
	...
	public String fetchAverageRating(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				averageRating = profileManagementService.getAverageRatings(
						companyId, CommonConstants.PROFILE_LEVEL_COMPANY,
						aggregateAbusive);
			} else if (entityType.equals(CommonConstants.REGION_ID_COLUMN)) {
				long regionId = Long
						.parseLong(request.getParameter("regionId"));
				...
				averageRating = profileManagementService.getAverageRatings(
						regionId, CommonConstants.PROFILE_LEVEL_REGION,
						aggregateAbusive);
			} else if (entityType.equals(CommonConstants.BRANCH_ID_COLUMN)) {
				long branchId = Long
						.parseLong(request.getParameter("branchId"));
				...
				averageRating = profileManagementService.getAverageRatings(
						branchId, CommonConstants.PROFILE_LEVEL_BRANCH,
						aggregateAbusive);
			} else if (entityType.equals(CommonConstants.AGENT_ID_COLUMN)) {
				long agentId = Long.parseLong(request.getParameter("agentId"));
				if (agentId == 0l) {
				...
				averageRating = profileManagementService.getAverageRatings(
						agentId, CommonConstants.PROFILE_LEVEL_INDIVIDUAL,
						aggregateAbusive);
			}
		} catch (InvalidInputException e) {
			throw new InternalServerException(new ProfileServiceErrorCode(
					...
					CommonConstants.SERVICE_CODE_COMPANY_REVIEWS,
					"Something went wrong while fetching Average rating"),
					e.getMessage());
		}
	...
	public String verifyAccount(@RequestParam("q") String encryptedUrlParams,
		...
		try {
			profileManagementService
					.updateEmailVerificationStatus(encryptedUrlParams);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.EMAIL_VERIFICATION_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (InvalidInputException e) {
			LOG.error("InvalidInputException while verifying email. Reason : "
					+ e.getMessage(), e);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.INVALID_VERIFICATION_URL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String saveStatus(HttpServletRequest request, Model model) {
		...
		try {
			...
			profileManagementService.addSocialPosts(user, entityId, entityType,
					text);
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while saving status of user. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.SAVE_STATUS_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String deleteStatus(HttpServletRequest request, Model model) {
		...
		try {
			...
			LOG.error(
					"NonFatalException while deleting status of user. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			return messageUtils.getDisplayMessage(
					DisplayMessageConstants.STATUS_CAN_NOT_BE_DELETED,
					DisplayMessageType.ERROR_MESSAGE).getMessage();
		}
		LOG.info("Method to delete status of the user finished");
		return messageUtils.getDisplayMessage(
				DisplayMessageConstants.STATUS_DELETED_SUCCESSFULLY,
	...
	public String getSocialPosts(HttpServletRequest request, Model model) {
		...
		try {
			...
			posts = profileManagementService.getSocialPosts(entityId,
					entityType, startIndex, batchSize);
		} catch (NonFatalException nonFatalException) {
			LOG.error("NonFatalException while fetching posts. Reason :"
					+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.FETCH_SOCIAL_POSTS_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String updateDisclaimer(Model model, HttpServletRequest request) {
		...
		try {
			...
			if (disclaimer == null || disclaimer.isEmpty()) {
				throw new InvalidInputException(
						"Disclaimer can not be null or empty",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			if (entityType.equals(CommonConstants.COMPANY_ID_COLUMN)) {
				...
				throw new InvalidInputException(
						"Error occurred while updating Disclaimer",
						DisplayMessageConstants.GENERAL_ERROR);
			}
			...
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.DISCLAIMER_UPDATE_SUCCESSFUL,
					DisplayMessageType.SUCCESS_MESSAGE));
		} catch (NonFatalException nonFatalException) {
			LOG.error(
					"NonFatalException while updating Disclaimer details. Reason :"
							+ nonFatalException.getMessage(), nonFatalException);
			model.addAttribute("message", messageUtils.getDisplayMessage(
					DisplayMessageConstants.DISCLAIMER_UPDATE_UNSUCCESSFUL,
					DisplayMessageType.ERROR_MESSAGE));
		}
	...
	public String getProfileUrlEditPage(Model model, HttpServletRequest request) {
		...
		UserSettings userSettings = (UserSettings) session
				.getAttribute(CommonConstants.CANONICAL_USERSETTINGS_IN_SESSION);
		String companyProfileUrl = userSettings.getCompanySettings()
				.getProfileUrl();
		switch (entityType) {
	...
	public String updatePositions(HttpServletRequest request, Model model) {
		...
		AgentSettings agentSettings = (AgentSettings) session
				.getAttribute(CommonConstants.USER_PROFILE_SETTINGS);

		String postionsJson = request.getParameter("positions");
		List<CompanyPositions> companyPositions = new Gson().fromJson(
				postionsJson, new TypeToken<List<CompanyPositions>>() {
				}.getType());

		profileManagementService.addOrUpdateAgentPositions(companyPositions,
				agentSettings);
