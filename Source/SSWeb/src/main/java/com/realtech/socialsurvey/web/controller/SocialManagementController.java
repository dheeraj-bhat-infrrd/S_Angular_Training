		...
		try {
			...
			try {
				...
				if (facebook.getId() != null) {
					...
					if (result != null && result.contains("url") && result.contains("\"is_silhouette\":false")) {
						...
						LOG.info("either profpic is default");
						profileImageUrl = "";
					}
				}
				FacebookPage personalUserAccount = new FacebookPage();
	...
	public String authenticateLinkedInAccessV2Image(Model model, HttpServletRequest request) {
		...
		try {
			...
			params.add(new BasicNameValuePair("redirect_uri",
					requestUtils.getRequestServerName(request) + "/linkedinauthimage.do"));
			...
			HttpGet httpGetImage = new HttpGet(
					"https://api.linkedin.com/v2/me?projection=(id,profilePicture(displayImage~:playableStreams))");
			httpGetImage.setHeader(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken);
			httpGetImage.setHeader(CommonConstants.X_RESTLI_PROTOCOL_VERSION,
			...
			if (basicProfileStrResponse != null && basicProfileStrResponse.contains("identifier")) {
				...
				basicProfileStrResponse = basicProfileStrResponse.substring(basicProfileStrResponse.indexOf("http"),
						basicProfileStrResponse.indexOf(',') - 1);
